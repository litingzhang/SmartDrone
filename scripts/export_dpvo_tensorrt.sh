...
mkdir -p "$(dirname "$PATCH_ONNX")" "$(dirname "$PATCH_ENGINE")" "$(dirname "$UPDATE_ONNX")" "$(dirname "$UPDATE_ENGINE")"
...
python3 - "$REPO" "$WEIGHTS" "$PATCH_ONNX" "$UPDATE_ONNX" "$WIDTH" "$HEIGHT" "$MAX_EDGES" <<'PY'
import os
import sys
...
repo, weights_path, patch_onnx, update_onnx = sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4]
width, height, max_edges = int(sys.argv[5]), int(sys.argv[6]), int(sys.argv[7])
...
for k, v in update_state.items():
    if k.startswith("d.1."):
        ...
        adapted[k] = v
missing, unexpected = update.load_state_dict(adapted, strict=False)
allowed_missing = set()
if unexpected:
