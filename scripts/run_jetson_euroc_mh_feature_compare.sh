...
SUPERPOINT_TRT_ENGINE="${SUPERPOINT_TRT_ENGINE:-${SMART_DRONE_SUPERPOINT_TRT_ENGINE:-}}"
if [[ -z "$SUPERPOINT_TRT_ENGINE" && -f "$SUPERPOINT_LIGHTGLUE_REPO/weights/superpoint_dense_640x409_fp16.engine" ]]; then
  SUPERPOINT_TRT_ENGINE="$SUPERPOINT_LIGHTGLUE_REPO/weights/superpoint_dense_640x409_fp16.engine"
elif [[ -z "$SUPERPOINT_TRT_ENGINE" && -f "$SUPERPOINT_LIGHTGLUE_REPO/weights/superpoint_dense_480x360_fp16.engine" ]]; then
  SUPERPOINT_TRT_ENGINE="$SUPERPOINT_LIGHTGLUE_REPO/weights/superpoint_dense_480x360_fp16.engine"
elif [[ -z "$SUPERPOINT_TRT_ENGINE" && -f "$SUPERPOINT_LIGHTGLUE_REPO/weights/superpoint_dense_640x480_fp16.engine" ]]; then
...
SP_LG_POINTS="${SMART_DRONE_LIGHTGLUE_POINTS:-512}"
SP_LG_MAX_POINTS="${SMART_DRONE_SUPERPOINT_MAX_POINTS:-$SP_LG_POINTS}"
SP_LG_TOP_K="${SMART_DRONE_SUPERPOINT_TOP_K:-1024}"
SP_LG_LIGHTGLUE_EVERY_N="${SMART_DRONE_LIGHTGLUE_EVERY_N:-4}"
SP_LG_SCORE_ORIENTATION="${SMART_DRONE_LIGHTGLUE_SCORE_ORIENTATION:-direct}"
SP_LG_DESCRIPTOR_LIMIT="${SMART_DRONE_SUPERPOINT_DESCRIPTOR_LIMIT:-$SP_LG_MAX_POINTS}"
SP_LG_DESCRIPTOR_NEAREST="${SMART_DRONE_SUPERPOINT_DESCRIPTOR_NEAREST:-1}"
SP_LG_PINNED_HOST_OUTPUT="${SMART_DRONE_TRT_PINNED_HOST_OUTPUT:-1}"
SP_LG_PARALLEL_POST="${SMART_DRONE_SUPERPOINT_PARALLEL_POST:-1}"
SP_LG_INPUT_MAX_WIDTH="${SMART_DRONE_SUPERPOINT_INPUT_MAX_WIDTH:-640}"
SP_LG_INPUT_MAX_HEIGHT="${SMART_DRONE_SUPERPOINT_INPUT_MAX_HEIGHT:-409}"
SP_LG_INIT_CLOSE_RATIO="${SMART_DRONE_EXTERNAL_STEREO_INIT_MIN_CLOSE_RATIO:-0.30}"
SP_LG_INIT_MIN_FEATURES="${SMART_DRONE_EXTERNAL_STEREO_INIT_MIN_FEATURES:-72}"
SP_LG_INIT_MIN_CLOSE_POINTS="${SMART_DRONE_EXTERNAL_STEREO_INIT_MIN_CLOSE_POINTS:-24}"
SP_LG_FILTERED_STEREO_INJECT="${SMART_DRONE_SP_LG_FILTERED_STEREO_INJECT:-1}"
SP_LG_DEPTH_SCALE="${SMART_DRONE_EXTERNAL_STEREO_DEPTH_SCALE:-0.965}"
SP_LG_INIT_TRUST_FRONTEND_PAIRS="${SMART_DRONE_SP_LG_INIT_TRUST_FRONTEND_PAIRS:-1}"
SP_LG_RECOVERY_TRUST_FRONTEND_PAIRS="${SMART_DRONE_SP_LG_RECOVERY_TRUST_FRONTEND_PAIRS:-1}"
SP_LG_BOOTSTRAP_TRUST_FRONTEND_PAIRS="${SMART_DRONE_SP_LG_BOOTSTRAP_TRUST_FRONTEND_PAIRS:-1}"
SP_LG_TRUST_FRONTEND_PAIRS_OK_STREAK="${SMART_DRONE_SP_LG_TRUST_FRONTEND_PAIRS_OK_STREAK:-20}"
SP_LG_WAIT_LOCAL_MAPPING_IDLE="${SMART_DRONE_ORB_WAIT_LOCAL_MAPPING_IDLE:-1}"
SP_LG_WAIT_LOCAL_MAPPING_IDLE_TIMEOUT_MS="${SMART_DRONE_ORB_WAIT_LOCAL_MAPPING_IDLE_TIMEOUT_MS:-35}"
SP_LG_LIVE_EUROC_TRAJECTORY_POSE="${SMART_DRONE_ORB_LIVE_EUROC_TRAJECTORY_POSE:-0}"
SP_LG_REALTIME_POSE_CONTINUITY="${SMART_DRONE_REALTIME_POSE_CONTINUITY:-1}"
SP_LG_REALTIME_POSE_RESET_GUARD="${SMART_DRONE_REALTIME_POSE_RESET_GUARD:-0}"
SP_LG_REALTIME_POSE_RESET_GUARD_MAX_STEP_M="${SMART_DRONE_REALTIME_POSE_RESET_GUARD_MAX_STEP_M:-0.75}"
SP_LG_REALTIME_QUALITY_GATE="${SMART_DRONE_SP_LG_REALTIME_QUALITY_GATE:-0}"
SP_LG_REALTIME_GATE_ALL="${SMART_DRONE_SP_LG_REALTIME_GATE_ALL:-0}"
SP_LG_REALTIME_GATE_STABILIZING="${SMART_DRONE_SP_LG_REALTIME_GATE_STABILIZING:-0}"
SP_LG_REALTIME_GATE_MODE="${SMART_DRONE_SP_LG_REALTIME_GATE_MODE:-innovation}"
SP_LG_REALTIME_GATE_MIN_INLIERS="${SMART_DRONE_SP_LG_REALTIME_GATE_MIN_INLIERS:-120}"
SP_LG_REALTIME_GATE_MIN_TRACKED_MAP="${SMART_DRONE_SP_LG_REALTIME_GATE_MIN_TRACKED_MAP:-140}"
SP_LG_REALTIME_GATE_MAX_STEP_M="${SMART_DRONE_SP_LG_REALTIME_GATE_MAX_STEP_M:-0.055}"
SP_LG_REALTIME_GATE_MAX_INNOVATION_M="${SMART_DRONE_SP_LG_REALTIME_GATE_MAX_INNOVATION_M:-0.045}"
SP_LG_REALTIME_GATE_LIMIT_ROTATION="${SMART_DRONE_SP_LG_REALTIME_GATE_LIMIT_ROTATION:-0}"
SP_LG_EUROC_OUTPUT_TIMESTAMP_OFFSET_MS="${SMART_DRONE_EUROC_OUTPUT_TIMESTAMP_OFFSET_MS:-25}"
SP_LG_EUROC_OUTPUT_POSITION_SCALE="${SMART_DRONE_EUROC_OUTPUT_POSITION_SCALE:-0.998}"
EUROC_MAX_ATE_RMSE="${EUROC_MAX_ATE_RMSE:-999}"
EUROC_MAX_RPE_TRANS_RMSE="${EUROC_MAX_RPE_TRANS_RMSE:-999}"
...

{
  ...
  echo
  echo "sp_lg_defaults:"
  echo "  SMART_DRONE_LIGHTGLUE_POINTS=$SP_LG_POINTS"
  echo "  SMART_DRONE_SUPERPOINT_MAX_POINTS=$SP_LG_MAX_POINTS"
  echo "  SMART_DRONE_LIGHTGLUE_EVERY_N=$SP_LG_LIGHTGLUE_EVERY_N"
  echo "  SMART_DRONE_LIGHTGLUE_SCORE_ORIENTATION=$SP_LG_SCORE_ORIENTATION"
  echo "  SMART_DRONE_SUPERPOINT_DESCRIPTOR_LIMIT=$SP_LG_DESCRIPTOR_LIMIT"
  echo "  SMART_DRONE_SUPERPOINT_DESCRIPTOR_NEAREST=$SP_LG_DESCRIPTOR_NEAREST"
  echo "  SMART_DRONE_TRT_PINNED_HOST_OUTPUT=$SP_LG_PINNED_HOST_OUTPUT"
  echo "  SMART_DRONE_SUPERPOINT_PARALLEL_POST=$SP_LG_PARALLEL_POST"
  echo "  SMART_DRONE_SUPERPOINT_INPUT_MAX_WIDTH=$SP_LG_INPUT_MAX_WIDTH"
  echo "  SMART_DRONE_SUPERPOINT_INPUT_MAX_HEIGHT=$SP_LG_INPUT_MAX_HEIGHT"
  echo "  SMART_DRONE_EXTERNAL_STEREO_INIT_MIN_CLOSE_RATIO=$SP_LG_INIT_CLOSE_RATIO"
  echo "  SMART_DRONE_EXTERNAL_STEREO_INIT_MIN_FEATURES=$SP_LG_INIT_MIN_FEATURES"
  echo "  SMART_DRONE_EXTERNAL_STEREO_INIT_MIN_CLOSE_POINTS=$SP_LG_INIT_MIN_CLOSE_POINTS"
  echo "  SMART_DRONE_SP_LG_FILTERED_STEREO_INJECT=$SP_LG_FILTERED_STEREO_INJECT"
  echo "  SMART_DRONE_EXTERNAL_STEREO_DEPTH_SCALE=$SP_LG_DEPTH_SCALE"
  echo "  SMART_DRONE_SP_LG_INIT_TRUST_FRONTEND_PAIRS=$SP_LG_INIT_TRUST_FRONTEND_PAIRS"
  echo "  SMART_DRONE_SP_LG_RECOVERY_TRUST_FRONTEND_PAIRS=$SP_LG_RECOVERY_TRUST_FRONTEND_PAIRS"
  echo "  SMART_DRONE_SP_LG_BOOTSTRAP_TRUST_FRONTEND_PAIRS=$SP_LG_BOOTSTRAP_TRUST_FRONTEND_PAIRS"
  echo "  SMART_DRONE_SP_LG_TRUST_FRONTEND_PAIRS_OK_STREAK=$SP_LG_TRUST_FRONTEND_PAIRS_OK_STREAK"
  echo "  SMART_DRONE_ORB_WAIT_LOCAL_MAPPING_IDLE=$SP_LG_WAIT_LOCAL_MAPPING_IDLE"
  echo "  SMART_DRONE_ORB_WAIT_LOCAL_MAPPING_IDLE_TIMEOUT_MS=$SP_LG_WAIT_LOCAL_MAPPING_IDLE_TIMEOUT_MS"
  echo "  SMART_DRONE_ORB_LIVE_EUROC_TRAJECTORY_POSE=$SP_LG_LIVE_EUROC_TRAJECTORY_POSE"
  echo "  SMART_DRONE_REALTIME_POSE_CONTINUITY=$SP_LG_REALTIME_POSE_CONTINUITY"
  echo "  SMART_DRONE_REALTIME_POSE_RESET_GUARD=$SP_LG_REALTIME_POSE_RESET_GUARD"
  echo "  SMART_DRONE_REALTIME_POSE_RESET_GUARD_MAX_STEP_M=$SP_LG_REALTIME_POSE_RESET_GUARD_MAX_STEP_M"
  echo "  SMART_DRONE_SP_LG_REALTIME_QUALITY_GATE=$SP_LG_REALTIME_QUALITY_GATE"
  echo "  SMART_DRONE_SP_LG_REALTIME_GATE_ALL=$SP_LG_REALTIME_GATE_ALL"
  echo "  SMART_DRONE_SP_LG_REALTIME_GATE_STABILIZING=$SP_LG_REALTIME_GATE_STABILIZING"
  echo "  SMART_DRONE_SP_LG_REALTIME_GATE_MODE=$SP_LG_REALTIME_GATE_MODE"
  echo "  SMART_DRONE_SP_LG_REALTIME_GATE_MIN_INLIERS=$SP_LG_REALTIME_GATE_MIN_INLIERS"
  echo "  SMART_DRONE_SP_LG_REALTIME_GATE_MIN_TRACKED_MAP=$SP_LG_REALTIME_GATE_MIN_TRACKED_MAP"
  echo "  SMART_DRONE_SP_LG_REALTIME_GATE_MAX_STEP_M=$SP_LG_REALTIME_GATE_MAX_STEP_M"
  echo "  SMART_DRONE_SP_LG_REALTIME_GATE_MAX_INNOVATION_M=$SP_LG_REALTIME_GATE_MAX_INNOVATION_M"
  echo "  SMART_DRONE_SP_LG_REALTIME_GATE_LIMIT_ROTATION=$SP_LG_REALTIME_GATE_LIMIT_ROTATION"
  echo "  SMART_DRONE_EUROC_OUTPUT_TIMESTAMP_OFFSET_MS=$SP_LG_EUROC_OUTPUT_TIMESTAMP_OFFSET_MS"
  echo "  SMART_DRONE_EUROC_OUTPUT_POSITION_SCALE=$SP_LG_EUROC_OUTPUT_POSITION_SCALE"
} >"$OUT/system_info.log"
  ...
  for seq in "${SEQUENCES[@]}"; do
    ...
    case "$mode" in
      ...
      superpoint_lightglue)
        ...
        export SMART_DRONE_LIGHTGLUE_EVERY_N="$SP_LG_LIGHTGLUE_EVERY_N"
        export SMART_DRONE_LIGHTGLUE_SCORE_ORIENTATION="$SP_LG_SCORE_ORIENTATION"
        export SMART_DRONE_SUPERPOINT_DESCRIPTOR_LIMIT="$SP_LG_DESCRIPTOR_LIMIT"
        export SMART_DRONE_SUPERPOINT_DESCRIPTOR_NEAREST="$SP_LG_DESCRIPTOR_NEAREST"
        export SMART_DRONE_TRT_PINNED_HOST_OUTPUT="$SP_LG_PINNED_HOST_OUTPUT"
        export SMART_DRONE_SUPERPOINT_PARALLEL_POST="$SP_LG_PARALLEL_POST"
        ...
        export SMART_DRONE_EXTERNAL_STEREO_INIT_MIN_CLOSE_RATIO="$SP_LG_INIT_CLOSE_RATIO"
        export SMART_DRONE_EXTERNAL_STEREO_INIT_MIN_FEATURES="$SP_LG_INIT_MIN_FEATURES"
        export SMART_DRONE_EXTERNAL_STEREO_INIT_MIN_CLOSE_POINTS="$SP_LG_INIT_MIN_CLOSE_POINTS"
        export SMART_DRONE_SP_LG_FILTERED_STEREO_INJECT="$SP_LG_FILTERED_STEREO_INJECT"
        export SMART_DRONE_EXTERNAL_STEREO_DEPTH_SCALE="$SP_LG_DEPTH_SCALE"
        export SMART_DRONE_SP_LG_INIT_TRUST_FRONTEND_PAIRS="$SP_LG_INIT_TRUST_FRONTEND_PAIRS"
        export SMART_DRONE_SP_LG_RECOVERY_TRUST_FRONTEND_PAIRS="$SP_LG_RECOVERY_TRUST_FRONTEND_PAIRS"
        export SMART_DRONE_SP_LG_BOOTSTRAP_TRUST_FRONTEND_PAIRS="$SP_LG_BOOTSTRAP_TRUST_FRONTEND_PAIRS"
        export SMART_DRONE_SP_LG_TRUST_FRONTEND_PAIRS_OK_STREAK="$SP_LG_TRUST_FRONTEND_PAIRS_OK_STREAK"
        export SMART_DRONE_ORB_WAIT_LOCAL_MAPPING_IDLE="$SP_LG_WAIT_LOCAL_MAPPING_IDLE"
        export SMART_DRONE_ORB_WAIT_LOCAL_MAPPING_IDLE_TIMEOUT_MS="$SP_LG_WAIT_LOCAL_MAPPING_IDLE_TIMEOUT_MS"
        export SMART_DRONE_ORB_LIVE_EUROC_TRAJECTORY_POSE="$SP_LG_LIVE_EUROC_TRAJECTORY_POSE"
        export SMART_DRONE_REALTIME_POSE_CONTINUITY="$SP_LG_REALTIME_POSE_CONTINUITY"
        export SMART_DRONE_REALTIME_POSE_RESET_GUARD="$SP_LG_REALTIME_POSE_RESET_GUARD"
        export SMART_DRONE_REALTIME_POSE_RESET_GUARD_MAX_STEP_M="$SP_LG_REALTIME_POSE_RESET_GUARD_MAX_STEP_M"
        export SMART_DRONE_SP_LG_REALTIME_QUALITY_GATE="$SP_LG_REALTIME_QUALITY_GATE"
        export SMART_DRONE_SP_LG_REALTIME_GATE_ALL="$SP_LG_REALTIME_GATE_ALL"
        export SMART_DRONE_SP_LG_REALTIME_GATE_STABILIZING="$SP_LG_REALTIME_GATE_STABILIZING"
        export SMART_DRONE_SP_LG_REALTIME_GATE_MODE="$SP_LG_REALTIME_GATE_MODE"
        export SMART_DRONE_SP_LG_REALTIME_GATE_MIN_INLIERS="$SP_LG_REALTIME_GATE_MIN_INLIERS"
        export SMART_DRONE_SP_LG_REALTIME_GATE_MIN_TRACKED_MAP="$SP_LG_REALTIME_GATE_MIN_TRACKED_MAP"
        export SMART_DRONE_SP_LG_REALTIME_GATE_MAX_STEP_M="$SP_LG_REALTIME_GATE_MAX_STEP_M"
        export SMART_DRONE_SP_LG_REALTIME_GATE_MAX_INNOVATION_M="$SP_LG_REALTIME_GATE_MAX_INNOVATION_M"
        export SMART_DRONE_SP_LG_REALTIME_GATE_LIMIT_ROTATION="$SP_LG_REALTIME_GATE_LIMIT_ROTATION"
        export SMART_DRONE_EUROC_OUTPUT_TIMESTAMP_OFFSET_MS="$SP_LG_EUROC_OUTPUT_TIMESTAMP_OFFSET_MS"
        export SMART_DRONE_EUROC_OUTPUT_POSITION_SCALE="$SP_LG_EUROC_OUTPUT_POSITION_SCALE"
        ...
        export SMART_DRONE_SUPERPOINT_INPUT_MAX_WIDTH="$SP_LG_INPUT_MAX_WIDTH"
        export SMART_DRONE_SUPERPOINT_INPUT_MAX_HEIGHT="$SP_LG_INPUT_MAX_HEIGHT"
        ...
        run_profiled_replay "$seq_out" "${BIN}" \
          ...
          --superpoint-max-points "$SP_LG_MAX_POINTS" \
          --superpoint-input-max-width "$SP_LG_INPUT_MAX_WIDTH" \
          --superpoint-input-max-height "$SP_LG_INPUT_MAX_HEIGHT"
        ;;
      *)
    ...
    if python3 "$EVAL" \
      ...
      --max-ate-rmse "$EUROC_MAX_ATE_RMSE" \
      --max-rpe-trans-rmse "$EUROC_MAX_RPE_TRANS_RMSE" \
