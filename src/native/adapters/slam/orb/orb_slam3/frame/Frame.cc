...
#include "adapters/slam/stereo/visual_frame_observation_loader.h"
...
inline int PatchL1Distance11x11(const cv::Mat &leftLevel, int leftCenterX,
                                int leftCenterY, const cv::Mat &rightLevel,
  ...
  auto accumulateRowSad = [&](const uchar *leftPtr, const uchar *rightPtr) {
    ...
    rowSad +=
        std::abs(static_cast<int>(leftPtr[8]) - static_cast<int>(rightPtr[8]));
    rowSad +=
        std::abs(static_cast<int>(leftPtr[9]) - static_cast<int>(rightPtr[9]));
    rowSad += std::abs(static_cast<int>(leftPtr[10]) -
                       static_cast<int>(rightPtr[10]));
    return rowSad;
  };

  int dy = -kHalfWindow;
  for (; dy + 3 <= kHalfWindow; dy += 4) {
    const uchar *leftPtr0 =
        leftLevel.ptr<uchar>(leftCenterY + dy) + (leftCenterX - kHalfWindow);
    const uchar *rightPtr0 =
        rightLevel.ptr<uchar>(rightCenterY + dy) + (rightCenterX - kHalfWindow);
    const uchar *leftPtr1 = leftLevel.ptr<uchar>(leftCenterY + dy + 1) +
                            (leftCenterX - kHalfWindow);
    const uchar *rightPtr1 = rightLevel.ptr<uchar>(rightCenterY + dy + 1) +
                             (rightCenterX - kHalfWindow);
    const uchar *leftPtr2 = leftLevel.ptr<uchar>(leftCenterY + dy + 2) +
                            (leftCenterX - kHalfWindow);
    const uchar *rightPtr2 = rightLevel.ptr<uchar>(rightCenterY + dy + 2) +
                             (rightCenterX - kHalfWindow);
    const uchar *leftPtr3 = leftLevel.ptr<uchar>(leftCenterY + dy + 3) +
                            (leftCenterX - kHalfWindow);
    const uchar *rightPtr3 = rightLevel.ptr<uchar>(rightCenterY + dy + 3) +
                             (rightCenterX - kHalfWindow);
  ...
  for (int dy = -kHalfWindow; dy <= kHalfWindow; ++dy) {
    ...
    for (int dx = 0; dx < kWindowSize; ++dx) {
      sad += std::abs(static_cast<int>(leftPtr[dx]) -
                      static_cast<int>(rightPtr[dx]));
    }
  }
#endif
  return sad;
...
float StereoFeatureDepthScale() {
  const char *value = EnvValueWithStereoFeatureFallback(
      "SMART_DRONE_STEREO_FEATURE_DEPTH_SCALE");
  if (!value || value[0] == '\0')
    return 1.0f;
...
bool StereoFeatureIgnoreProjectionScaleLevels() {
  const char *value = EnvValueWithStereoFeatureFallback(
      "SMART_DRONE_STEREO_FEATURE_IGNORE_PROJECTION_SCALE_LEVELS");
  if (!value || value[0] == '\0')
    return false;
...
int StereoFeatureKeypointOctave() {
  const char *value = EnvValueWithStereoFeatureFallback(
      "SMART_DRONE_STEREO_FEATURE_KEYPOINT_OCTAVE");
  if (!value || value[0] == '\0')
    return 0;
...
void LogStereoMatchDebug(int leftCount, int rightCount, int rowLinked,
                         int accepted, int uniqueMatches, int finalMatches,
                         float median, float threshold) {
  ...
  std::cerr << "[stereo_match_debug] frame=" << index
            << " left_kp=" << leftCount << " right_kp=" << rightCount
            << " row_linked=" << rowLinked << " accepted=" << accepted
            << " unique=" << uniqueMatches << " final=" << finalMatches
            << " median=" << median << " th=" << threshold << std::endl;
}
...
Frame::Frame(const Frame &frame)
    : mpcpi(frame.mpcpi), mpORBvocabulary(frame.mpORBvocabulary),
      ...
      mbImuPreintegrated(frame.mbImuPreintegrated), mpCamera(frame.mpCamera),
...
Frame::Frame(const cv::Mat &imLeft, const cv::Mat &imRight,
  ...
  if (mpORBextractorLeft->LastExtractionUsedCuda() &&
...
Frame::Frame(const cv::Mat &imLeft, const cv::Mat &imRight,
             ...
             ORBextractor *extractorRight, ORBVocabulary *voc, cv::Mat &K,
             cv::Mat &distCoef, const float &bf, const float &thDepth,
             GeometricCamera *pCamera, const StereoFeatureFrameData &features,
  ...
  LoadStereoFeatureFrameData(features);
  if (mvKeys.empty())
    return;
...
Frame::Frame(const cv::Mat &imGray, const double &timeStamp,
             ORBextractor *extractor, ORBVocabulary *voc,
             GeometricCamera *pCamera, cv::Mat &distCoef, const float &bf,
             const float &thDepth, const MonoFeatureFrameData &features,
             Frame *pPrevF, const IMU::Calib &ImuCalib)
    : mpcpi(NULL), mpORBvocabulary(voc), mpORBextractorLeft(extractor),
      mpORBextractorRight(static_cast<ORBextractor *>(NULL)),
      mTimeStamp(timeStamp), mK(static_cast<Pinhole *>(pCamera)->toK()),
  ...
  LoadMonoFeatureFrameData(features);
  if (mvKeys.empty())
    return;
...
void Frame::LoadStereoFeatureFrameData(const StereoFeatureFrameData &features) {
  ...
  mDescriptorsRight = std::move(observation.rightDescriptors);
  N = observation.featureCount;
  mvuRight = std::move(observation.rightU);
  mvDepth = std::move(observation.depth);
  mnCloseMPs = observation.closePointCount;
  Nleft = -1;
  Nright = -1;
  monoLeft = -1;
  monoRight = -1;
...
bool Frame::ShouldIgnoreScaleLevelsForProjection() const {
  return mbStereoFeatureInjected && StereoFeatureIgnoreProjectionScaleLevels();
}
...
void Frame::ComputeBoW() {
  if (mDescriptors.empty() || mDescriptors.type() != CV_8U ||
...
void Frame::ComputeStereoMatches() {
  ...
  const bool cudaLevel0StereoRefinement =
      mpORBextractorLeft != nullptr && mpORBextractorRight != nullptr &&
      mpORBextractorLeft->LastExtractionUsedCuda() &&
      mpORBextractorRight->LastExtractionUsedCuda() &&
      !ForceCudaStereoPatchRefinement() && !imgLeft.empty() &&
      !imgRight.empty();
  ...
  const int thOrbDist =
      (kOrbMatcherHighDistance + kOrbMatcherLowDistance) / 2;

  int nRows = 0;
  if (mpORBextractorLeft != nullptr &&
  ...
  const size_t rowReserve =
      std::max<size_t>(32, (static_cast<size_t>(Nr) * 3) / std::max(1, nRows));
  ...
  for (int iL = 0; iL < N; iL++) {
    ...
    int bestDist = kOrbMatcherHighDistance;
    int bestDist2 = kOrbMatcherHighDistance;
    size_t bestIdxR = 0;
    ...
    for (size_t iC = 0; iC < vCandidates.size(); iC++) {
      ...
      if (uR >= minU && uR <= maxU) {
        ...
        const int dist = ComputeOrbDescriptorDistance(dL, dR);
    ...
    if (bestDist >= kOrbMatcherHighDistance)
      continue;

    if (bestDist2 < kOrbMatcherHighDistance &&
        static_cast<float>(bestDist) > 0.9f * static_cast<float>(bestDist2))
      continue;
    ...
    if (bestDist < thOrbDist) {
      ...
      const float scaleFactor =
          cudaLevel0StereoRefinement ? 1.0f : mvInvScaleFactors[kpL.octave];
      ...
      const int w = 5;
      const cv::Mat &leftLevel =
          cudaLevel0StereoRefinement
              ? imgLeft
              : mpORBextractorLeft->mvImagePyramid[kpL.octave];
      const cv::Mat &rightLevel =
          cudaLevel0StereoRefinement
              ? imgRight
      ...
      if (iniu < 0 || endu >= rightLevel.cols || scaledvL - w < 0 ||
          scaledvL + w >= rightLevel.rows)
        continue;

      for (int incR = -L; incR <= +L; incR++) {
        const int rightCenterX = static_cast<int>(scaleduR0) + incR;
        const float dist = static_cast<float>(PatchL1Distance11x11(
            leftLevel, static_cast<int>(scaleduL), static_cast<int>(scaledvL),
      ...
      const float levelScale =
          cudaLevel0StereoRefinement ? 1.0f : mvScaleFactors[kpL.octave];
      ...
      if (disparity >= minD && disparity < maxD) {
        ...
        if (bestDist < bestRightCost[bestIdxR]) {
          ...
          bestCandidateForRight[bestIdxR] =
              static_cast<int>(vCandidatesAccepted.size());
          vCandidatesAccepted.push_back(
              {iL, static_cast<int>(bestIdxR), bestDist, bestuR, depth});
        }
      }
    }
  }
  ...
  if (vDistIdx.empty()) {
    LogStereoMatchDebug(N, Nr, rowLinked,
                        static_cast<int>(vCandidatesAccepted.size()), 0, 0,
                        -1.0f, -1.0f);
    return;
  }
  ...
  LogStereoMatchDebug(N, Nr, rowLinked, static_cast<int>(vDistIdx.size()),
                      static_cast<int>(vDistIdx.size()), finalMatches, median,
                      thDist);
}
