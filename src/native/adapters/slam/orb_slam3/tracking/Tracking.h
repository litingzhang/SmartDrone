...
public:
  ...
  Sophus::SE3f GrabImageMonocular(const cv::Mat &im, const double &timestamp,
                                  string filename);
  Sophus::SE3f
  GrabImageStereoWithFeatures(const cv::Mat &imRectLeft,
                              const cv::Mat &imRectRight,
                              const StereoFeatureFrameData &features,
                              const double &timestamp, string filename);
  Sophus::SE3f
  GrabImageMonocularWithFeatures(const cv::Mat &im,
                                 const MonoFeatureFrameData &features,
                                 const double &timestamp, string filename);
  ...
  ExtractTrackedPointCloud(size_t maxPointCloudPoints) const;
  ...
  int GetTrackingState() const { return static_cast<int>(mState); }
  SlamFrameTrackingStats GetFrameTrackingStats() const;
  std::vector<OrbFrameTrajectoryEntry> GetFrameTrajectorySnapshot() const;
  bool GetLatestFrameTrajectoryEntry(OrbFrameTrajectoryEntry &entry) const;
...
public:
  ...
  int mnStereoFeatureInitFrameId = -1;
...
protected:
  ...
  OrbFrameSensorContext BuildFrameSensorContext(
      Frame *previousFrame = nullptr,
      const IMU::Calib *imuCalibration = nullptr) const;
  bool NeedsInitExtractor() const;
  ...
  std::unique_ptr<IOrbFrameFactory> mpFrameFactory;
