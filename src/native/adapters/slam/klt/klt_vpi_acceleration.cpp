...
bool FailVpiStateSetup(std::shared_ptr<LkPerFrameVpiState> &state,
                       bool &logged, const char *message, VPIStatus status,
                       const char *fallback)
{
    std::cerr << "[lk_per_frame_accel] " << message << ": "
              << VpiStatusName(status) << "; fallback=" << fallback << "\n";
    state.reset();
    logged = true;
    return false;
}
...
void ResetVpiPerFrameState(std::shared_ptr<LkPerFrameVpiState> &state,
                           const cv::Size &size, int maxDisparity)
{
...
bool CreateVpiPerFrameStream(std::shared_ptr<LkPerFrameVpiState> &state,
                             bool &logged)
{
    const VPIStatus status = vpiStreamCreate(VPI_BACKEND_CUDA, &state->stream);
    if (status == VPI_SUCCESS) {
        return true;
    }
    return FailVpiStateSetup(state, logged, "VPI CUDA stream create failed",
                             status, "cpu_sgbm");
}
                             ...
                             bool &logged)
{
    ...
    VPIStatus status =
        vpiCreateRemap(VPI_BACKEND_CUDA, &state->leftWarp,
                       &state->leftRemapPayload);
    if (status == VPI_SUCCESS) {
        status = vpiCreateRemap(VPI_BACKEND_CUDA, &state->rightWarp,
                                &state->rightRemapPayload);
    }
    if (status == VPI_SUCCESS) {
        return true;
    }
...
bool CreateVpiRectifiedImages(std::shared_ptr<LkPerFrameVpiState> &state,
                              const cv::Size &size, bool &logged)
{
    const uint64_t imageBackends = VPI_BACKEND_CUDA | VPI_BACKEND_CPU;
    VPIStatus status = vpiImageCreate(size.width, size.height,
                                      VPI_IMAGE_FORMAT_U8, imageBackends,
                                      &state->leftRect);
    if (status == VPI_SUCCESS) {
        status = vpiImageCreate(size.width, size.height, VPI_IMAGE_FORMAT_U8,
                                imageBackends, &state->rightRect);
    }
    if (status == VPI_SUCCESS) {
        status = vpiImageCreate(size.width, size.height, VPI_IMAGE_FORMAT_U8,
                                imageBackends, &state->prevLeftRect);
    }
    if (status == VPI_SUCCESS) {
        status = vpiImageCreate(size.width, size.height, VPI_IMAGE_FORMAT_U8,
                                imageBackends, &state->prevRightRect);
    }
    if (status == VPI_SUCCESS) {
        return true;
    }
    return FailVpiStateSetup(state, logged,
                             "VPI rectified image create failed", status,
                             "cpu_sgbm");
}

bool CreateVpiStereoPayload(std::shared_ptr<LkPerFrameVpiState> &state,
                            const cv::Size &size, int maxDisparity,
                            bool &logged)
{
    ...
    if (status != VPI_SUCCESS) {
        return FailVpiStateSetup(
            state, logged, "VPI stereo creation params init failed", status,
            "cpu_sgbm");
    }
    ...
    return FailVpiStateSetup(state, logged,
                             "VPI CUDA stereo payload create failed", status,
                             "cpu_sgbm");
}

bool CreateVpiDisparityImage(std::shared_ptr<LkPerFrameVpiState> &state,
                             const cv::Size &size, bool &logged)
{
    const VPIStatus status = vpiImageCreate(
        size.width, size.height, VPI_IMAGE_FORMAT_S16,
        VPI_BACKEND_CUDA | VPI_BACKEND_CPU, &state->disparity);
    if (status == VPI_SUCCESS) {
        return true;
    }
    return FailVpiStateSetup(state, logged,
                             "VPI disparity image create failed", status,
                             "cpu_sgbm");
}

bool CreateVpiPyramidResources(std::shared_ptr<LkPerFrameVpiState> &state,
                               const cv::Size &size, bool &logged)
{
    VPIStatus status = vpiPyramidCreate(
        size.width, size.height, VPI_IMAGE_FORMAT_U8, 4, 0.5f,
        VPI_BACKEND_CUDA | VPI_BACKEND_CPU, &state->prevPyr);
    if (status == VPI_SUCCESS) {
        status = vpiPyramidCreate(
            size.width, size.height, VPI_IMAGE_FORMAT_U8, 4, 0.5f,
    ...
    return FailVpiStateSetup(state, logged, "VPI pyramid create failed",
                             status, "cpu_lk");
}

bool CreateVpiPyrLkPayload(std::shared_ptr<LkPerFrameVpiState> &state,
                           const cv::Size &size, bool &logged)
{
    const VPIStatus status = vpiCreateOpticalFlowPyrLK(
        VPI_BACKEND_CUDA, size.width, size.height, VPI_IMAGE_FORMAT_U8, 4,
        0.5f, &state->lkPayload);
    if (status == VPI_SUCCESS) {
        return true;
    }
    return FailVpiStateSetup(state, logged, "VPI PyrLK payload create failed",
                             status, "cpu_lk");
}

bool CreateVpiLkArrays(std::shared_ptr<LkPerFrameVpiState> &state,
                       bool &logged)
{
    ...
    return FailVpiStateSetup(state, logged, "VPI LK array create failed",
                             status, "cpu_lk");
}

void LogVpiPerFrameStateReady(const cv::Size &size, int maxDisparity)
{
    std::cerr << "[lk_per_frame_accel] backend=vpi_cuda "
              << "stages=remap,stereo_disparity,pyr_lk"
                            ...
                            int maxDisparity, bool &logged)
{
    ...
    if (!CreateVpiPerFrameStream(state, logged) ||
        ...
        !CreateVpiPyramidResources(state, size, logged) ||
        !CreateVpiPyrLkPayload(state, size, logged) ||
        !CreateVpiLkArrays(state, logged)) {
        return false;
    }

    LogVpiPerFrameStateReady(size, maxDisparity);
    logged = true;
    return true;
}
...
bool HasValidPyrLkState(const std::shared_ptr<LkPerFrameVpiState> &state,
                        VPIImage curLeftImage,
                        const std::vector<cv::Point2f> &pts0)
{
    return state != nullptr && state->stream != nullptr &&
           state->lkPayload != nullptr && state->prevPyr != nullptr &&
           state->curPyr != nullptr && state->prevPts != nullptr &&
           state->curPts != nullptr && state->trackStatus != nullptr &&
           curLeftImage != nullptr && !pts0.empty();
}

bool UploadVpiPyrLkPoints(const std::vector<cv::Point2f> &pts0, int count,
                          std::shared_ptr<LkPerFrameVpiState> &state)
{
    VPIArrayData prevData{};
    VPIArrayData curInitData{};
    VPIStatus status = vpiArrayLockData(
        state->prevPts, VPI_LOCK_WRITE, VPI_ARRAY_BUFFER_HOST_AOS, &prevData);
    if (status != VPI_SUCCESS || prevData.bufferType != VPI_ARRAY_BUFFER_HOST_AOS ||
        prevData.buffer.aos.data == nullptr ||
        prevData.buffer.aos.sizePointer == nullptr) {
        std::cerr << "[lk_per_frame_accel] VPI prev point upload lock failed: "
                  << VpiStatusName(status) << "; fallback=cpu_lk\n";
        return false;
    }
    status = vpiArrayLockData(state->curPts, VPI_LOCK_WRITE,
                              VPI_ARRAY_BUFFER_HOST_AOS, &curInitData);
    if (status != VPI_SUCCESS ||
        curInitData.bufferType != VPI_ARRAY_BUFFER_HOST_AOS ||
        curInitData.buffer.aos.data == nullptr ||
        curInitData.buffer.aos.sizePointer == nullptr) {
        std::cerr << "[lk_per_frame_accel] VPI current point init lock failed: "
                  << VpiStatusName(status) << "; fallback=cpu_lk\n";
    ...
    auto *curInitKeypoints =
        static_cast<VPIKeypointF32 *>(curInitData.buffer.aos.data);
...
VPIStatus CreatePreviousPyrLkImageWrapper(const cv::Mat &prevLeft,
                                          VPIImage prevLeftImage,
                                          VPIImage &prevWrapper,
                                          VPIImage &prevImage)
{
    prevWrapper = nullptr;
    prevImage = prevLeftImage;
    if (prevImage != nullptr) {
        return VPI_SUCCESS;
    }
    const VPIStatus status = vpiImageCreateWrapperOpenCVMat(
        prevLeft, VPI_IMAGE_FORMAT_U8, VPI_BACKEND_CUDA, &prevWrapper);
    prevImage = prevWrapper;
    return status;
}

VPIStatus SubmitVpiPyrLkPyramids(const cv::Mat &prevLeft,
                                 VPIImage prevLeftImage,
                                 VPIImage curLeftImage,
                                 VPIImage &prevWrapper,
                                 std::shared_ptr<LkPerFrameVpiState> &state)
{
    VPIImage prevImage = nullptr;
    VPIStatus status = CreatePreviousPyrLkImageWrapper(
        prevLeft, prevLeftImage, prevWrapper, prevImage);
    if (status == VPI_SUCCESS) {
        status = vpiSubmitGaussianPyramidGenerator(
            state->stream, VPI_BACKEND_CUDA, prevImage, state->prevPyr,
            VPI_BORDER_CLAMP);
    }
    if (status == VPI_SUCCESS) {
        status = vpiSubmitGaussianPyramidGenerator(
            state->stream, VPI_BACKEND_CUDA, curLeftImage, state->curPyr,
            VPI_BORDER_CLAMP);
    }
    return status;
}

VPIStatus SubmitVpiPyrLkFlow(std::shared_ptr<LkPerFrameVpiState> &state)
{
    VPIOpticalFlowPyrLKParams lkParams{};
    VPIStatus status = vpiInitOpticalFlowPyrLKParams(&lkParams);
    if (status != VPI_SUCCESS) {
        return status;
    }
    ...
    status = vpiSubmitOpticalFlowPyrLK(
        state->stream, VPI_BACKEND_CUDA, state->lkPayload, state->prevPyr,
...
bool RunVpiPyrLk(const cv::Mat &prevLeft, VPIImage prevLeftImage,
                 VPIImage curLeftImage,
                 std::shared_ptr<LkPerFrameVpiState> &state)
{
    VPIImage prevWrapper = nullptr;
    VPIStatus status = SubmitVpiPyrLkPyramids(
        prevLeft, prevLeftImage, curLeftImage, prevWrapper, state);
    if (status == VPI_SUCCESS) {
        status = SubmitVpiPyrLkFlow(state);
    }
    ...
    std::cerr << "[lk_per_frame_accel] VPI PyrLK submit failed: "
              << VpiStatusName(status) << "; fallback=cpu_lk\n";
    return false;
}

bool DownloadVpiPyrLkResult(int count, std::vector<cv::Point2f> &pts1,
                            ...
                            std::shared_ptr<LkPerFrameVpiState> &state)
{
    ...
    VPIStatus status = vpiArrayLockData(
        state->curPts, VPI_LOCK_READ, VPI_ARRAY_BUFFER_HOST_AOS, &curData);
    if (status == VPI_SUCCESS) {
        status = vpiArrayLockData(state->trackStatus, VPI_LOCK_READ,
                                  VPI_ARRAY_BUFFER_HOST_AOS, &statusData);
    }
    if (status != VPI_SUCCESS || curData.buffer.aos.data == nullptr ||
        statusData.buffer.aos.data == nullptr) {
        std::cerr << "[lk_per_frame_accel] VPI LK result lock failed: "
                  << VpiStatusName(status) << "; fallback=cpu_lk\n";
    ...
    for (int i = 0; i < count; ++i) {
        pts1[static_cast<size_t>(i)] =
            cv::Point2f(curKeypoints[i].x, curKeypoints[i].y);
                                       ...
                                       bool &logged)
{
    ...
    if (status != VPI_SUCCESS) {
        if (!logged) {
            std::cerr << "[lk_per_frame_accel] VPI CUDA stream create failed: "
                      << VpiStatusName(status) << "; fallback=cpu_sgbm\n";
            logged = true;
        }
        state.reset();
        return false;
    ...
    if (status != VPI_SUCCESS) {
        return FailVpiStateSetup(
            state, logged, "VPI CUDA stereo payload create failed", status,
            "cpu_sgbm");
    }

    if (!CreateVpiDisparityImage(state, left.size(), logged)) {
        return false;
    }
    return true;
}

bool CreateStandaloneVpiDisparityWrappers(
    const cv::Mat &left, const cv::Mat &right,
    std::shared_ptr<LkPerFrameVpiState> &state, bool &logged)
{
    VPIStatus status = vpiImageCreateWrapperOpenCVMat(
        left, VPI_IMAGE_FORMAT_U8, VPI_BACKEND_CUDA, &state->leftWrapper);
    if (status == VPI_SUCCESS) {
        status = vpiImageCreateWrapperOpenCVMat(
            right, VPI_IMAGE_FORMAT_U8, VPI_BACKEND_CUDA,
            &state->rightWrapper);
    }
    if (status == VPI_SUCCESS) {
        return true;
    ...
    return FailVpiStateSetup(state, logged,
                             "VPI OpenCV wrapper create failed", status,
                             "cpu_sgbm");
}

void LogStandaloneVpiDisparityReady(const cv::Mat &left, int maxDisparity)
{
    std::cerr << "[lk_per_frame_accel] backend=vpi_cuda "
              << "stage=stereo_disparity size=" << left.cols << "x"
              << left.rows << " max_disparity=" << maxDisparity << "\n";
}

bool CreateStandaloneVpiDisparityResources(
    const cv::Mat &left, const cv::Mat &right, int maxDisparity,
    std::shared_ptr<LkPerFrameVpiState> &state, bool &logged)
{
    if (!CreateStandaloneVpiDisparityState(left, right, maxDisparity, state,
                                           logged) ||
        !CreateStandaloneVpiDisparityWrappers(left, right, state, logged)) {
        return false;
    }
    LogStandaloneVpiDisparityReady(left, maxDisparity);
    logged = true;
    return true;
}

VPIStatus UpdateVpiImageWrapper(VPIImage &wrapper, const cv::Mat &image)
{
    if (wrapper == nullptr) {
        return vpiImageCreateWrapperOpenCVMat(
            image, VPI_IMAGE_FORMAT_U8, VPI_BACKEND_CUDA, &wrapper);
    }
    return vpiImageSetWrappedOpenCVMat(wrapper, image);
}

bool UpdateStandaloneVpiDisparityWrappers(
    const cv::Mat &left, const cv::Mat &right,
    std::shared_ptr<LkPerFrameVpiState> &state)
{
    VPIStatus status = UpdateVpiImageWrapper(state->leftWrapper, left);
    if (status == VPI_SUCCESS) {
        status = UpdateVpiImageWrapper(state->rightWrapper, right);
    }
    if (status == VPI_SUCCESS) {
        return true;
    }
    std::cerr << "[lk_per_frame_accel] VPI wrapper update failed: "
              << VpiStatusName(status) << "; fallback=cpu_sgbm\n";
    state.reset();
    return false;
}

bool EnsureStandaloneVpiDisparityResources(
    const cv::Mat &left, const cv::Mat &right, int maxDisparity,
    std::shared_ptr<LkPerFrameVpiState> &state, bool &logged)
{
    const bool recreate = !state || state->width != left.cols ||
                          state->height != left.rows ||
                          state->maxDisparity != maxDisparity;
    if (recreate) {
        return CreateStandaloneVpiDisparityResources(left, right, maxDisparity,
                                                     state, logged);
    }
    return UpdateStandaloneVpiDisparityWrappers(left, right, state);
}

bool SubmitStandaloneVpiDisparity(
    int maxDisparity, std::shared_ptr<LkPerFrameVpiState> &state)
{
    VPIStereoDisparityEstimatorParams params{};
    VPIStatus status = vpiInitStereoDisparityEstimatorParams(&params);
    if (status != VPI_SUCCESS) {
        std::cerr << "[lk_per_frame_accel] VPI stereo params init failed: "
                  << VpiStatusName(status) << "; fallback=cpu_sgbm\n";
        return false;
    }
    ...
    status = vpiSubmitStereoDisparityEstimator(
        state->stream, VPI_BACKEND_CUDA, state->stereoPayload,
    ...
    std::cerr << "[lk_per_frame_accel] VPI stereo submit failed: "
              << VpiStatusName(status) << "; fallback=cpu_sgbm\n";
    return false;
}
                             ...
                             std::shared_ptr<LkPerFrameVpiState> &state, bool &logged)
{
    ...
    if (!EnsureStandaloneVpiDisparityResources(left, right, maxDisparity,
                                               state, logged) ||
        !SubmitStandaloneVpiDisparity(maxDisparity, state)) {
        return false;
    }
