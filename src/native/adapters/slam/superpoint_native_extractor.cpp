...
{
    return std::clamp(EnvFloat("SMART_DRONE_EXTERNAL_STEREO_MIN_DISPARITY_PX", kStereoMinDisparityPx),
...
struct TensorBlob {
    ...
    bool Empty() const { return dims.empty() || FloatData() == nullptr || FloatElementCount() == 0; }
    const float *FloatData() const { return floatData != nullptr ? floatData : data.data(); }
    size_t FloatElementCount() const { return floatData != nullptr ? floatElementCount : data.size(); }
...
struct CudaPinnedHostBuffer {
    ...
    CudaPinnedHostBuffer(CudaPinnedHostBuffer &&other) noexcept : ptr(other.ptr), bytes(other.bytes)
    {
        other.ptr = nullptr;
        other.bytes = 0;
    }
...
void BuildInputBatch(const std::vector<cv::Mat> &images, int targetHeight, int targetWidth,
...
const cv::Mat &SuperPointNmsKernel()
{
    static const cv::Mat kernel = cv::getStructuringElement(
        cv::MORPH_RECT, cv::Size(kSuperPointNmsRadius * 2 + 1, kSuperPointNmsRadius * 2 + 1));
    return kernel;
}
...
void BuildHeatmap(const TensorBlob &detector, int batch, cv::Mat &heatmap)
{
    ...
    const size_t imageStride = static_cast<size_t>(channels) * static_cast<size_t>(gridHeight) *
                               static_cast<size_t>(gridWidth);
    const size_t channelStride = static_cast<size_t>(gridHeight) * static_cast<size_t>(gridWidth);
    const size_t batchOffset = static_cast<size_t>(batch) * imageStride;
    const float *data = detector.FloatData();
    heatmap.create(gridHeight * kSuperPointCellSize, gridWidth * kSuperPointCellSize, CV_32F);
    heatmap.setTo(cv::Scalar(0.0f));
    for (int gy = 0; gy < gridHeight; ++gy) {
        for (int gx = 0; gx < gridWidth; ++gx) {
            const size_t spatialOffset = static_cast<size_t>(gy) * static_cast<size_t>(gridWidth) +
                                         static_cast<size_t>(gx);
            float maxLogit = data[batchOffset + spatialOffset];
            for (int c = 1; c < 65; ++c) {
                maxLogit = std::max(maxLogit, data[batchOffset + static_cast<size_t>(c) * channelStride +
                                                   spatialOffset]);
            }
            ...
            for (int c = 0; c < 65; ++c) {
                expValues[static_cast<size_t>(c)] =
                    std::exp(data[batchOffset + static_cast<size_t>(c) * channelStride + spatialOffset] -
                             maxLogit);
...
void ExtractCandidates(const TensorBlob &detector, int batch, int targetWidth, int targetHeight, int maxPoints,
                       SuperPointPostScratch &scratch, SuperPointPostStats *stats)
{
...
void ExtractCandidatesFastNms(const TensorBlob &detector, int batch, int targetWidth, int targetHeight,
                              int maxPoints, SuperPointPostScratch &scratch, SuperPointPostStats *stats)
{
    const int channels = detector.Dim(1);
    const int gridHeight = detector.Dim(2);
    const int gridWidth = detector.Dim(3);
        ...
        for (int gx = 0; gx < gridWidth; ++gx) {
            ...
            for (int c = 0; c < 64; ++c) {
                ...
                if (x < kSuperPointBorder || y < kSuperPointBorder ||
                    x >= targetWidth - kSuperPointBorder || y >= targetHeight - kSuperPointBorder) {
                    continue;
                }
    ...
    scratch.suppressionMask.assign(static_cast<size_t>(targetWidth) * static_cast<size_t>(targetHeight), 0);
    scratch.nmsCandidates.clear();
    scratch.nmsCandidates.reserve(static_cast<size_t>(std::max(1, maxPoints)));
    for (const Candidate &candidate : candidates) {
        const size_t center = static_cast<size_t>(candidate.y) * static_cast<size_t>(targetWidth) +
                              static_cast<size_t>(candidate.x);
        if (scratch.suppressionMask[center] != 0) {
            continue;
        }
        scratch.nmsCandidates.push_back(candidate);
        const int y0 = std::max(kSuperPointBorder, candidate.y - kSuperPointNmsRadius);
        const int y1 = std::min(targetHeight - kSuperPointBorder - 1, candidate.y + kSuperPointNmsRadius);
        const int x0 = std::max(kSuperPointBorder, candidate.x - kSuperPointNmsRadius);
        const int x1 = std::min(targetWidth - kSuperPointBorder - 1, candidate.x + kSuperPointNmsRadius);
        for (int y = y0; y <= y1; ++y) {
            uint8_t *row = scratch.suppressionMask.data() + static_cast<size_t>(y) *
                                                              static_cast<size_t>(targetWidth);
            for (int x = x0; x <= x1; ++x) {
                row[x] = 1;
            }
        }
        if (static_cast<int>(scratch.nmsCandidates.size()) >= std::max(1, maxPoints)) {
            break;
        }
    }
...
void SampleDescriptorBilinear(const TensorBlob &descriptors, int batch, float x, float y, float *out)
{
    ...
    const float w10 = (1.0f - wx) * wy;
    const float w11 = wx * wy;
    const size_t batchOffset = static_cast<size_t>(batch) * static_cast<size_t>(channels) *
                               static_cast<size_t>(height) * static_cast<size_t>(width);
    const size_t channelStride = static_cast<size_t>(height) * static_cast<size_t>(width);
    const float *data = descriptors.FloatData();
    for (int c = 0; c < kSuperPointDescriptorDim; ++c) {
        const size_t channelOffset = batchOffset + static_cast<size_t>(c) * channelStride;
        const float v00 = data[channelOffset + static_cast<size_t>(y0) * width + x0];
        const float v01 = data[channelOffset + static_cast<size_t>(y0) * width + x1];
        const float v10 = data[channelOffset + static_cast<size_t>(y1) * width + x0];
        const float v11 = data[channelOffset + static_cast<size_t>(y1) * width + x1];
        out[c] = w00 * v00 + w01 * v01 + w10 * v10 + w11 * v11;
    }
    NormalizeVector(out, kSuperPointDescriptorDim);
}

void SampleDescriptorNearest(const TensorBlob &descriptors, int batch, float x, float y, float *out)
{
    const int channels = descriptors.Dim(1);
    const int height = descriptors.Dim(2);
    const int width = descriptors.Dim(3);
    ...
    const size_t batchOffset = static_cast<size_t>(batch) * static_cast<size_t>(channels) *
                               static_cast<size_t>(height) * static_cast<size_t>(width);
    const size_t channelStride = static_cast<size_t>(height) * static_cast<size_t>(width);
    const size_t spatialOffset = static_cast<size_t>(iy) * static_cast<size_t>(width) +
                                 static_cast<size_t>(ix);
    const float *data = descriptors.FloatData();
    for (int c = 0; c < kSuperPointDescriptorDim; ++c) {
        out[c] = data[batchOffset + static_cast<size_t>(c) * channelStride + spatialOffset];
    }
    NormalizeVector(out, kSuperPointDescriptorDim);
}

void BuildDescriptorGridHwc(const TensorBlob &descriptors, int batch, std::vector<float> &hwc)
{
    const int channels = descriptors.Dim(1);
    const int height = descriptors.Dim(2);
    const int width = descriptors.Dim(3);
    const size_t spatial = static_cast<size_t>(height) * static_cast<size_t>(width);
    const size_t batchOffset = static_cast<size_t>(batch) * static_cast<size_t>(channels) * spatial;
    const float *data = descriptors.FloatData();
    hwc.resize(spatial * static_cast<size_t>(channels));

    constexpr int kTilePixels = 32;
    for (int tileStart = 0; tileStart < static_cast<int>(spatial); tileStart += kTilePixels) {
        const int tileEnd = std::min(tileStart + kTilePixels, static_cast<int>(spatial));
...
void SampleDescriptorBilinearHwc(const std::vector<float> &hwc, int height, int width, float x, float y,
                                 float *out)
{
    const float gx = std::clamp(x, 0.0f, static_cast<float>(width - 1));
    const float gy = std::clamp(y, 0.0f, static_cast<float>(height - 1));
    ...
    const size_t base00 = (static_cast<size_t>(y0) * static_cast<size_t>(width) +
                           static_cast<size_t>(x0)) * kSuperPointDescriptorDim;
    const size_t base01 = (static_cast<size_t>(y0) * static_cast<size_t>(width) +
                           static_cast<size_t>(x1)) * kSuperPointDescriptorDim;
                      ...
                      SuperPointFeatureSet &leftOut, SuperPointFeatureSet &rightOut)
{
    ...
    const int candidateLimit = EnvIntClamped("SMART_DRONE_DESCRIPTOR_SUPPLEMENT_CANDIDATES", std::max(1, maxPoints),
                                             1, 4096);
    const int leftCount =
        std::min({static_cast<int>(leftRaw.keypoints.size()), leftRaw.descriptors.rows, candidateLimit});
    const int rightCount =
        std::min({static_cast<int>(rightRaw.keypoints.size()), rightRaw.descriptors.rows, candidateLimit});
    ...
    for (int li = 0; li < leftCount; ++li) {
        ...
        const auto firstRight = std::lower_bound(
            rightOrder.begin(), rightOrder.end(), minY, [&](int index, float value) {
        ...
        for (auto it = firstRight; it != lastRight; ++it) {
            ...
            if (disparity < ExternalStereoMinDisparityPx() || disparity > kStereoMaxDisparityPx) {
                continue;
            }
...
bool IsStereoFeaturePairNearExisting(const cv::Point2f &leftPoint, const cv::Point2f &rightPoint,
                                     ...
                                     const SuperPointFeatureSet &existingRight)
{
    ...
    const size_t count = std::min(existingLeft.keypoints.size(), existingRight.keypoints.size());
    for (size_t i = 0; i < count; ++i) {
        ...
        if ((leftDelta.x * leftDelta.x + leftDelta.y * leftDelta.y) <= minDistanceSq &&
            (rightDelta.x * rightDelta.x + rightDelta.y * rightDelta.y) <= minDistanceSq) {
            return true;
        }
    }
    return false;
...
#if defined(SMART_DRONE_SUPERPOINT_TENSORRT_AVAILABLE)
bool ScheduleTensorRtOutputCopy(void *devicePtr, nvinfer1::DataType dtype, int64_t volume, cudaStream_t stream,
                                TensorBlob &output, CudaPinnedHostBuffer *pinnedHostBuffer,
                                TensorRtForwardStats *stats, const char *engineName, std::string *err)
{
    ...
    const bool usePinnedHost = pinnedHostBuffer != nullptr &&
                               EnvFlag("SMART_DRONE_TRT_PINNED_HOST_OUTPUT", false);
    auto recordOutputCopy = [&](const std::chrono::steady_clock::time_point &start,
                                const std::chrono::steady_clock::time_point &end,
                                size_t bytes, bool pinned) {
        if (stats != nullptr) {
    ...
    if (dtype == nvinfer1::DataType::kFLOAT) {
        ...
        if (cudaMemcpyAsync(hostPtr, devicePtr, bytes, cudaMemcpyDeviceToHost, stream) != cudaSuccess) {
            if (err != nullptr) {
                *err = std::string("TensorRT failed to copy ") + engineName + " FP32 output";
            }
            return false;
        }
    ...
    if (dtype == nvinfer1::DataType::kHALF) {
        ...
        if (cudaMemcpyAsync(hostPtr, devicePtr, bytes, cudaMemcpyDeviceToHost, stream) != cudaSuccess) {
            if (err != nullptr) {
                *err = std::string("TensorRT failed to copy ") + engineName + " FP16 output";
            }
            return false;
        }
        const auto outputEndTp = std::chrono::steady_clock::now();
    ...
    if (dtype == nvinfer1::DataType::kINT32) {
        ...
        if (cudaMemcpyAsync(hostPtr, devicePtr, bytes, cudaMemcpyDeviceToHost, stream) != cudaSuccess) {
            if (err != nullptr) {
                *err = std::string("TensorRT failed to copy ") + engineName + " INT32 output";
            }
            return false;
        }
        const auto outputEndTp = std::chrono::steady_clock::now();
    ...
    if (err != nullptr) {
        *err = std::string("TensorRT ") + engineName + " output has unsupported data type";
    }
    return false;
}
...
bool FinalizeTensorRtOutput(TensorBlob &output, TensorRtForwardStats *stats)
{
    ...
    if (output.hostStorage == TensorBlob::HostStorage::Float) {
        if (output.pinnedHostData && EnvFlag("SMART_DRONE_TRT_PINNED_HOST_VIEW", false)) {
            ...
            if (output.pinnedHostData) {
                std::memcpy(output.data.data(), output.pendingHostData, output.pendingElementCount * sizeof(float));
            }
...
void AppendStereoFeaturePairs(SuperPointFeatureSet &leftOut, SuperPointFeatureSet &rightOut,
                              const SuperPointFeatureSet &leftSupplement,
                              const SuperPointFeatureSet &rightSupplement, int maxPoints)
{
    if (leftSupplement.descriptors.empty() || rightSupplement.descriptors.empty() ||
        leftSupplement.descriptors.type() != CV_32F || rightSupplement.descriptors.type() != CV_32F ||
        leftSupplement.descriptors.cols != rightSupplement.descriptors.cols ||
        leftSupplement.descriptors.rows != static_cast<int>(leftSupplement.keypoints.size()) ||
        rightSupplement.descriptors.rows != static_cast<int>(rightSupplement.keypoints.size())) {
        return;
    }
    if ((!leftOut.descriptors.empty() && leftOut.descriptors.cols != leftSupplement.descriptors.cols) ||
        (!rightOut.descriptors.empty() && rightOut.descriptors.cols != rightSupplement.descriptors.cols)) {
        return;
    }

    const int limit = std::max(1, maxPoints);
    const size_t sourceCount = std::min(leftSupplement.keypoints.size(), rightSupplement.keypoints.size());
    for (size_t i = 0; i < sourceCount && static_cast<int>(leftOut.keypoints.size()) < limit; ++i) {
        const cv::Point2f &leftPoint = leftSupplement.keypoints[i];
        const cv::Point2f &rightPoint = rightSupplement.keypoints[i];
        if (IsStereoFeaturePairNearExisting(leftPoint, rightPoint, leftOut, rightOut)) {
            continue;
        }
        leftOut.keypoints.push_back(leftPoint);
        rightOut.keypoints.push_back(rightPoint);
        leftOut.descriptors.push_back(leftSupplement.descriptors.row(static_cast<int>(i)));
        rightOut.descriptors.push_back(rightSupplement.descriptors.row(static_cast<int>(i)));
    }
}
...
class TensorRtSuperPointEngine {
  public:
    ...
    bool Forward(const std::vector<float> &batch, int batchSize, int height, int width, TensorBlob &detector,
                 TensorBlob &descriptors, TensorRtForwardStats *stats, std::string *err)
    {
        if (stats != nullptr) {
        ...
        if (!ReadOutput(m_detectorIndex, detector, stats, err) || !ReadOutput(m_descriptorIndex, descriptors, stats, err)) {
            return false;
        }
        if (stats != nullptr && m_eventTimingEnabled) {
        ...
        if (stats != nullptr) {
            ...
            if (m_eventTimingEnabled) {
                ...
                float outputMs = 0.0f;
                if (cudaEventElapsedTime(&computeMs, m_computeStartEvent, m_computeEndEvent) == cudaSuccess) {
                    stats->gpuComputeMs += static_cast<double>(computeMs);
                }
                if (cudaEventElapsedTime(&outputMs, m_outputStartEvent, m_outputEndEvent) == cudaSuccess) {
    ...
    bool SupportsBatchSize(int batchSize) const
    {
        ...
        for (int profile = 0; profile < profiles; ++profile) {
            const nvinfer1::Dims minDims =
                m_engine->getProfileDimensions(m_inputIndex, profile, nvinfer1::OptProfileSelector::kMIN);
            const nvinfer1::Dims maxDims =
                m_engine->getProfileDimensions(m_inputIndex, profile, nvinfer1::OptProfileSelector::kMAX);
            if (minDims.nbDims == 4 && maxDims.nbDims == 4 && minDims.d[0] <= batchSize &&
                batchSize <= maxDims.d[0]) {
                return true;
            }
        }
        return batchSize == 1;
  ...
  private:
    ...
    bool EnsureBindingBuffer(int index, const nvinfer1::Dims &dims, nvinfer1::DataType type, std::string *err)
    {
        ...
        if (m_bindings.size() < static_cast<size_t>(m_engine->getNbBindings())) {
            ...
            m_pinnedHostOutputs.resize(static_cast<size_t>(m_engine->getNbBindings()));
        }
    ...
    bool ReadOutput(int index, TensorBlob &output, TensorRtForwardStats *stats, std::string *err)
    {
        const nvinfer1::Dims dims = m_context->getBindingDimensions(index);
        ...
        return ScheduleTensorRtOutputCopy(m_bindings[static_cast<size_t>(index)], dtype, volume, m_stream, output,
                                          &m_pinnedHostOutputs[static_cast<size_t>(index)], stats, "SuperPoint", err);
    }
...
class TensorRtLightGlueEngine {
  public:
    ...
    bool Forward(const std::vector<float> &keypoints0, const std::vector<float> &keypoints1,
                 ...
                 TensorBlob &scores, TensorRtForwardStats *stats, std::string *err)
    {
        if (stats != nullptr) {
            *stats = TensorRtForwardStats{};
            stats->eventTimingEnabled = m_eventTimingEnabled;
        ...
        if (!SetInput(m_kpts0Index, {1, pointCount, 2}, keypoints0, stats, err) ||
            ...
            !SetInput(m_desc0Index, {1, pointCount, kSuperPointDescriptorDim}, descriptors0, stats, err) ||
            !SetInput(m_desc1Index, {1, pointCount, kSuperPointDescriptorDim}, descriptors1, stats, err)) {
            return false;
        }
  ...
  private:
    ...
    bool SetInput(int index, const std::vector<int> &dims, const std::vector<float> &data,
    ...
    bool ReadOutput(int index, TensorBlob &output, TensorRtForwardStats *stats, std::string *err)
    {
        ...
        const nvinfer1::DataType dtype = m_engine->getBindingDataType(index);
        return ScheduleTensorRtOutputCopy(m_bindings[static_cast<size_t>(index)], dtype, volume, m_stream, output,
                                          &m_pinnedHostOutputs[static_cast<size_t>(index)], stats, "LightGlue", err);
    }
...
struct SuperPointNativeExtractor::Impl {
    ...
    void ParallelPostWorkerLoop()
    {
        while (true) {
            ...
            int descriptorLimit = 0;
            {
                ...
                parallelPostCv.wait(lock, [this]() { return parallelPostStop || parallelPostHasJob; });
            ...
            const bool ok = detectorBlob != nullptr && descriptorBlob != nullptr && sourceImage != nullptr &&
                            output != nullptr && postStats != nullptr &&
                            PopulateOutputFromTensors(*detectorBlob, *descriptorBlob, tensorBatch, *sourceImage,
    ...
    void StartParallelPostJob(const TensorBlob &detectorBlob, const TensorBlob &descriptorBlob, int tensorBatch,
                              const cv::Mat &sourceImage, int targetHeight, int targetWidth, int maxPoints,
                              int descriptorLimit, SuperPointFeatureSet &output, SuperPointPostStats &postStats)
    {
    ...
    bool Load(const std::string &repoPath, const std::string &deviceText, std::string *err)
    {
        ...
        if (!lightGluePath.empty()) {
            ...
            if (matcher->Load(lightGluePath, &lgErr)) {
                ...
                lightGlueLowYieldMinPairs = EnvInt("SMART_DRONE_LIGHTGLUE_LOW_YIELD_MIN_PAIRS", 8);
                ...
                std::cerr << "[lightglue_trt] loaded engine=" << lightGluePath.string()
                          ...
                          << " low_yield_disable_threshold=" << lightGlueLowYieldDisableThreshold
                ...
                std::cerr << "[lightglue_trt] warning: failed to load engine=" << lightGluePath.string()
                          << " err=" << lgErr << "; sp_descriptor=primary\n";
            }
        } else {
    ...
    int maxPointsForLightGlue() const { return EnvInt("SMART_DRONE_LIGHTGLUE_POINTS", 512); }

    bool PopulateOutputFromTensors(const TensorBlob &detectorBlob, const TensorBlob &descriptorBlob, int tensorBatch,
                                   const cv::Mat &sourceImage, int targetHeight, int targetWidth, int maxPoints,
                                   int descriptorLimit, SuperPointFeatureSet &output,
                                   ...
                                   SuperPointPostStats *postStats, std::string *err)
    {
        ...
        if (detectorBlob.dims.size() < 4 || descriptorBlob.dims.size() < 4 ||
            detectorBlob.Dim(0) <= tensorBatch || descriptorBlob.Dim(0) <= tensorBatch ||
            detectorBlob.Dim(1) < 65 || descriptorBlob.Dim(1) != kSuperPointDescriptorDim) {
            if (err != nullptr) {
        ...
        const double ratioH = static_cast<double>(sourceImage.rows) / static_cast<double>(targetHeight);
        const double ratioW = static_cast<double>(sourceImage.cols) / static_cast<double>(targetWidth);
        SuperPointPostStats imagePostStats;
        if (EnvFlag("SMART_DRONE_SUPERPOINT_FAST_NMS", false)) {
            ExtractCandidatesFastNms(detectorBlob, tensorBatch, heatmapWidth, heatmapHeight, maxPoints, scratch,
                                     &imagePostStats);
        } else {
            ExtractCandidates(detectorBlob, tensorBatch, heatmapWidth, heatmapHeight, maxPoints, scratch,
                              &imagePostStats);
        }
        ...
        const int descriptorCount =
            std::min(static_cast<int>(candidates.size()), std::clamp(descriptorLimit, 0, maxPoints));
        if (descriptorCount > 0) {
            output.descriptors = cv::Mat(descriptorCount, kSuperPointDescriptorDim, CV_32F);
        }
        const auto descriptorStartTp = std::chrono::steady_clock::now();
        const bool useDescriptorHwc = EnvFlag("SMART_DRONE_SUPERPOINT_DESCRIPTOR_HWC", false);
        const bool useDescriptorNearest = EnvFlag("SMART_DRONE_SUPERPOINT_DESCRIPTOR_NEAREST", false);
        if (useDescriptorHwc && descriptorCount > 0) {
            BuildDescriptorGridHwc(descriptorBlob, tensorBatch, scratch.descriptorHwc);
        }
        for (size_t i = 0; i < candidates.size(); ++i) {
            ...
            const float sampleX =
                ...
                (static_cast<float>(descriptorBlob.Dim(3) * kSuperPointCellSize) - kSuperPointCellSize / 2.0f -
                 0.5f) *
                static_cast<float>(descriptorBlob.Dim(3) - 1);
            const float sampleY =
                (static_cast<float>(candidate.y) - kSuperPointCellSize / 2.0f + 0.5f) /
                (static_cast<float>(descriptorBlob.Dim(2) * kSuperPointCellSize) - kSuperPointCellSize / 2.0f -
                 0.5f) *
                static_cast<float>(descriptorBlob.Dim(2) - 1);
            if (useDescriptorNearest) {
                SampleDescriptorNearest(descriptorBlob, tensorBatch, sampleX, sampleY, descriptor);
            } else if (useDescriptorHwc) {
                SampleDescriptorBilinearHwc(scratch.descriptorHwc, descriptorBlob.Dim(2), descriptorBlob.Dim(3),
                                            sampleX, sampleY, descriptor);
            } else {
                SampleDescriptorBilinear(descriptorBlob, tensorBatch, sampleX, sampleY, descriptor);
            }
        }
        const auto descriptorEndTp = std::chrono::steady_clock::now();
        imagePostStats.descriptorMs += DurationMs(descriptorStartTp, descriptorEndTp);
    ...
    bool DetectAndComputeBatch(const std::vector<cv::Mat> &grayImages, int maxPoints, int descriptorLimit,
                               ...
                               double *postMs, std::string *err)
    {
        ...
        const int batchTargetHeight = inputHeight > 0 ? inputHeight : grayImages.front().rows;
        const int batchTargetWidth = inputWidth > 0 ? inputWidth : grayImages.front().cols;
        if (batchSize > 1 && !superPointBatchDisabled && trtEngine->SupportsBatchSize(batchSize)) {
            const auto inputStartTp = std::chrono::steady_clock::now();
            BuildInputBatch(grayImages, batchTargetHeight, batchTargetWidth, superPointInputBatch);
            const auto inputEndTp = std::chrono::steady_clock::now();
            const auto forwardStartTp = inputEndTp;
            std::string batchErr;
            TensorRtForwardStats batchStats;
            if (trtEngine->Forward(superPointInputBatch, batchSize, batchTargetHeight, batchTargetWidth, detector,
                                   descriptors, &batchStats, &batchErr)) {
                const auto forwardEndTp = std::chrono::steady_clock::now();
                const auto postStartTp = forwardEndTp;
                if (EnvFlag("SMART_DRONE_SUPERPOINT_PARALLEL_POST", false) && grayImages.size() == 2) {
                    std::array<SuperPointPostStats, 2> postStats{};
                    std::array<std::string, 2> postErrors{};
                    StartParallelPostJob(detector, descriptors, 1, grayImages[1], batchTargetHeight,
                                         batchTargetWidth, maxPoints, descriptorLimit, outputs[1], postStats[1]);
                    const bool leftOk = PopulateOutputFromTensors(
                        detector, descriptors, 0, grayImages[0], batchTargetHeight, batchTargetWidth, maxPoints,
                        descriptorLimit, outputs[0], mainPostScratch, &postStats[0], &postErrors[0]);
                    ...
                    for (size_t batchIndex = 0; batchIndex < grayImages.size(); ++batchIndex) {
                        if (!PopulateOutputFromTensors(detector, descriptors, static_cast<int>(batchIndex),
        ...
        for (size_t batchIndex = 0; batchIndex < grayImages.size(); ++batchIndex) {
            ...
            const auto inputStartTp = std::chrono::steady_clock::now();
            BuildInputBatch({grayImages[batchIndex]}, targetHeight, targetWidth, superPointInputBatch);
            const auto inputEndTp = std::chrono::steady_clock::now();
            const auto forwardStartTp = inputEndTp;
            TensorRtForwardStats singleStats;
            if (!trtEngine->Forward(superPointInputBatch, 1, targetHeight, targetWidth, detector, descriptors,
                                    &singleStats, err)) {
                return false;
            }
            const auto forwardEndTp = std::chrono::steady_clock::now();
            const auto postStartTp = forwardEndTp;
            if (!PopulateOutputFromTensors(detector, descriptors, 0, grayImages[batchIndex], targetHeight,
            ...
            lastSuperPointForwardStats.eventTimingEnabled =
                lastSuperPointForwardStats.eventTimingEnabled || singleStats.eventTimingEnabled;
                            ...
                            double *matchMs, std::string *err)
    {
        ...
        auto packInputs = [&](int pointCount) {
            ...
            const size_t descSize = static_cast<size_t>(pointCount) * kSuperPointDescriptorDim;
            ...
            for (int i = 0; i < leftCopyCount; ++i) {
                lightGlueKpts0[static_cast<size_t>(i) * 2] = leftRaw.keypoints[static_cast<size_t>(i)].x;
                lightGlueKpts0[static_cast<size_t>(i) * 2 + 1] = leftRaw.keypoints[static_cast<size_t>(i)].y;
                const float *src = leftRaw.descriptors.ptr<float>(i);
                std::copy(src, src + kSuperPointDescriptorDim,
                          lightGlueDesc0.data() + static_cast<size_t>(i) * kSuperPointDescriptorDim);
            }
            for (int i = 0; i < rightCopyCount; ++i) {
                lightGlueKpts1[static_cast<size_t>(i) * 2] = rightRaw.keypoints[static_cast<size_t>(i)].x;
                lightGlueKpts1[static_cast<size_t>(i) * 2 + 1] = rightRaw.keypoints[static_cast<size_t>(i)].y;
                const float *src = rightRaw.descriptors.ptr<float>(i);
                std::copy(src, src + kSuperPointDescriptorDim,
                          lightGlueDesc1.data() + static_cast<size_t>(i) * kSuperPointDescriptorDim);
            }
        };

        const int requestedPointCount =
            std::min({std::max(leftCount, rightCount), maxPoints, lightGluePointCount});
        int lightGlueInputPointCount =
            lightGlueDynamicPointCountDisabled ? lightGluePointCount : requestedPointCount;
        ...
        if (!lightGlueEngine->Forward(lightGlueKpts0, lightGlueKpts1, lightGlueDesc0, lightGlueDesc1, imageSize,
                                      imageSize, lightGlueInputPointCount, lightGlueScores, &matchStats, err)) {
            if (lightGlueInputPointCount < lightGluePointCount) {
                ...
                lastLightGlueInputPointCount = lightGlueInputPointCount;
                packInputs(lightGlueInputPointCount);
                if (!lightGlueEngine->Forward(lightGlueKpts0, lightGlueKpts1, lightGlueDesc0, lightGlueDesc1,
        ...
        auto scoreAtDirect = [&](int li, int ri) {
            return lightGlueScores.FloatData()[static_cast<size_t>(li) * static_cast<size_t>(matrixCols) +
        ...
        auto buildPairs = [&](bool transpose, int *mutualCount, int *scorePassCount, int *geometryPassCount) {
            std::vector<Pair> candidatePairs;
            std::vector<int> bestRightForLeft(static_cast<size_t>(outLeftCount), -1);
            std::vector<float> bestScoreForLeft(static_cast<size_t>(outLeftCount),
                                                -std::numeric_limits<float>::infinity());
            std::vector<int> bestLeftForRight(static_cast<size_t>(outRightCount), -1);
            std::vector<float> bestScoreForRight(static_cast<size_t>(outRightCount),
                                                 -std::numeric_limits<float>::infinity());
        ...
        const std::string orientation = LowerCopy(
            std::getenv("SMART_DRONE_LIGHTGLUE_SCORE_ORIENTATION") != nullptr ?
                std::getenv("SMART_DRONE_LIGHTGLUE_SCORE_ORIENTATION") : "auto");
        const bool useDirect = orientation != "transpose";
        const bool useTranspose = orientation != "direct";
        std::vector<Pair> directPairs;
        if (useDirect) {
            directPairs = buildPairs(false, &directMutualCount, &directScorePassCount, &directGeometryPassCount);
        }
        ...
        if (useTranspose) {
            transposePairs = buildPairs(true, &transposeMutualCount, &transposeScorePassCount,
        ...
        if (useTranspose && (!useDirect || transposeGeometryPassCount > directGeometryPassCount)) {
...
bool SuperPointNativeExtractor::DetectAndCompute(const cv::Mat &gray, SuperPointFeatureSet &outFeatures, std::string *err)
{
    ...
    if (!m_impl->DetectAndComputeBatch({gray8}, detectMaxPoints, detectMaxPoints, outputs, &inputMs, &forwardMs,
                                       &postMs, err) ||
        outputs.empty()) {
        return false;
    }
                                                       ...
                                                       std::string *err)
{
    ...
    const int descriptorCandidates =
        EnvIntClamped("SMART_DRONE_DESCRIPTOR_SUPPLEMENT_CANDIDATES", requestedMaxPoints, 1, 4096);
    const int requiredFeatureBudget =
        std::max({requestedMaxPoints, lightGluePoints, descriptorCandidates}) + kSuperPointStereoExtractionSlack;
    const int extractionBudget =
        EnvIntClamped("SMART_DRONE_SUPERPOINT_STEREO_EXTRACTION_BUDGET",
                      std::min(std::max(m_topK, requestedMaxPoints), requiredFeatureBudget), requestedMaxPoints, 4096);
    const int descriptorLimit =
        EnvIntClamped("SMART_DRONE_SUPERPOINT_DESCRIPTOR_LIMIT",
                      std::max({requestedMaxPoints, lightGluePoints, descriptorCandidates}), 1, extractionBudget);
    if (!m_impl->DetectAndComputeBatch({leftGray8, rightGray8}, extractionBudget, descriptorLimit, rawOutputs,
    ...
    const int lightGlueEveryN =
        m_lightGlueEveryNOverride > 0 ? std::clamp(m_lightGlueEveryNOverride, 1, 120)
    ...
    const int lightGlueSupplementMinPairs = EnvIntClamped("SMART_DRONE_LIGHTGLUE_SUPPLEMENT_MIN_PAIRS",
                                                          kLightGlueMinStereoPairsForSupplement, 0, maxStereoPairs);
    auto buildDescriptorMatches = [&]() {
        if (!descriptorLeftFeatures.keypoints.empty() || !descriptorRightFeatures.keypoints.empty()) {
            return;
        }
        const auto descriptorMatchStartTp = std::chrono::steady_clock::now();
        MatchStereoPairs(rawOutputs[0], rawOutputs[1], maxStereoPairs, descriptorLeftFeatures, descriptorRightFeatures);
        const auto descriptorMatchEndTp = std::chrono::steady_clock::now();
        descriptorMatchMs += DurationMs(descriptorMatchStartTp, descriptorMatchEndTp);
        ++descriptorMatchCalls;
        descriptorLeftCount = static_cast<int>(descriptorLeftFeatures.keypoints.size());
        descriptorRightCount = static_cast<int>(descriptorRightFeatures.keypoints.size());
    };
    ...
    if (!skippedLightGlue &&
        m_impl->MatchWithLightGlue(rawOutputs[0], rawOutputs[1], maxStereoPairs, leftGray8.cols,
                                   leftGray8.rows, lightGlueLeftFeatures, lightGlueRightFeatures, &lightGlueMatchMs, err)) {
        usedLightGlue = true;
        if (!lightGlueLeftFeatures.keypoints.empty() && !lightGlueRightFeatures.keypoints.empty()) {
            lightGlueLeftCount = static_cast<int>(lightGlueLeftFeatures.keypoints.size());
            lightGlueRightCount = static_cast<int>(lightGlueRightFeatures.keypoints.size());
            ...
            if (lightGlueLeftCount < m_impl->lightGlueLowYieldMinPairs) {
                ...
                if (m_impl->lightGlueLowYieldCount >= m_impl->lightGlueLowYieldDisableThreshold) {
                    m_impl->lightGlueSkipRemaining = m_impl->lightGlueEmptyCooldownFrames;
                    m_impl->lightGlueLowYieldCount = 0;
                    std::cerr << "[lightglue_trt] low_yield_cooldown pairs=" << lightGlueLeftCount
            ...
            if (lightGlueSupplementMinPairs > 0 &&
                static_cast<int>(std::min(leftFeatures.keypoints.size(), rightFeatures.keypoints.size())) <
                    lightGlueSupplementMinPairs) {
                const size_t beforeCount = std::min(leftFeatures.keypoints.size(), rightFeatures.keypoints.size());
                buildDescriptorMatches();
                AppendStereoFeaturePairs(leftFeatures, rightFeatures, descriptorLeftFeatures, descriptorRightFeatures,
                                         maxStereoPairs);
                usedDescriptorSupplement =
                    std::min(leftFeatures.keypoints.size(), rightFeatures.keypoints.size()) > beforeCount;
            }
        } else {
            ++m_impl->lightGlueEmptyCount;
            ++m_impl->lightGlueLowYieldCount;
            if (m_impl->lightGlueEmptyCount >= m_impl->lightGlueEmptyDisableThreshold) {
                m_impl->lightGlueSkipRemaining = m_impl->lightGlueEmptyCooldownFrames;
                m_impl->lightGlueEmptyCount = 0;
                m_impl->lightGlueLowYieldCount = 0;
    ...
    m_lastStats.stereoRightCount = static_cast<int>(rightFeatures.keypoints.size());
    m_lastStats.lightGlueUsed = usedLightGlue;
    m_lastStats.descriptorFallbackUsed = usedDescriptorPrimary || usedDescriptorSupplement;
    ...
    std::cerr << "[superpoint_trt_perf] batch=2 input_ms=" << inputMs << " gpu_forward_ms=" << forwardMs
              ...
              << " sp_d2h_bytes=" << spStats.d2hBytes
              << " sp_batched=" << (m_impl->lastSuperPointBatchedForward ? "Y" : "N")
              ...
              << " lg_static_shape_fallback=" << (m_impl->lastLightGlueStaticShapeFallback ? "Y" : "N")
              << " sp_descriptor="
              << (usedDescriptorPrimary ? "primary" : (usedDescriptorSupplement ? "supplement" : "none"))
              ...
              << " lg_score_range=" << m_impl->lastLightGlueMinScore << "/" << m_impl->lastLightGlueMaxScore
              << " lg_score_space=" << (m_impl->lastLightGlueScoresLookLog ? "log" : "prob")
              ...
              << " raw_pts=" << rawOutputs[0].keypoints.size() << "/" << rawOutputs[1].keypoints.size()
              << " lg_pts=" << lightGlueLeftCount << "/" << lightGlueRightCount
              << " desc_pts=" << descriptorLeftCount << "/" << descriptorRightCount
              << " stereo_pts=" << leftFeatures.keypoints.size() << "/" << rightFeatures.keypoints.size()
              << "\n";
    return true;
#else
    if (err != nullptr) {
