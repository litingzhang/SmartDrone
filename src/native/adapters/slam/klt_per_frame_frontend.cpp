...
KltPerFrameAccelerationRequest ResolveKltPerFrameAccelerationRequest(const SlamModeSharedState &state)
{
    ...
    request.requestVpiRemap = request.requestVpi && EnvFlagEnabled("SMART_DRONE_VPI_REMAP", true);
    request.requestVpiDisparity = request.requestVpi && EnvFlagEnabled("SMART_DRONE_VPI_DISPARITY", true);
    request.requestVpiLk = request.requestVpi && EnvFlagEnabled("SMART_DRONE_VPI_LK", true);
    return request;
}

void StoreKltPerFramePreviousRectifiedIfNeeded(SlamModeSharedState &state,
    ...
    SlamModeSharedState &state, const cv::Mat &leftRaw, const cv::Mat &rightRaw)
{
    ...
    const KltPerFrameAccelerationRequest request = ResolveKltPerFrameAccelerationRequest(state);
    result.preferAcceleratedPnpDefaults = request.requestVpi;
    result.useKeyframeReference = EnvFlagEnabled("SMART_DRONE_LK_PER_FRAME_KEYFRAME", false);
    result.keyframeInterval = std::max(1, EnvIntValue("SMART_DRONE_LK_PER_FRAME_KEYFRAME_INTERVAL", 6));

    cv::Mat leftRect = leftGray;
    cv::Mat rightRect = rightGray;
    if (request.requestVpiRemap && !state.m_lkMap1x.empty() && !state.m_lkMap2x.empty()) {
        result.usedVpiRemap =
            VpiRemapCurrentStereo(leftGray, rightGray, leftRect, rightRect, state.m_lkPerFrameVpi,
                                  state.m_lkMap1x, state.m_lkMap1y, state.m_lkMap2x, state.m_lkMap2y,
                                  state.m_lkPerFrameAccelLogged);
    }
    if (!result.usedVpiRemap && !state.m_lkMap1x.empty() && !state.m_lkMap2x.empty()) {
    ...
    result.rectifyMs = std::chrono::duration<double, std::milli>(rectifyEnd - rectifyStart).count();
    result.inputPrepareMs = std::chrono::duration<double, std::milli>(rectifyEnd - prepareStart).count();
    ...
    if (result.usedVpiRemap && request.requestVpiDisparity &&
        ...
        usedVpiDisparity =
            ComputeVpiCudaPreviousRectifiedDisparity(state.m_lkPrevLeft.size(), disparity,
                                                    state.m_lkPerFrameVpi);
    }
#endif
    if (!usedVpiDisparity && request.requestVpiDisparity) {
        usedVpiDisparity =
            ComputeVpiCudaDisparity(state.m_lkPrevLeft, state.m_lkPrevRight, disparity,
    ...
    if (!usedVpiDisparity) {
        ...
        if (!state.m_lkPerFrameSgbm) {
            const int numDisparities = std::max(16, ((result.leftRect.cols / 8 + 15) / 16) * 16);
            state.m_lkPerFrameSgbm =
                cv::StereoSGBM::create(0, numDisparities, 5, 8 * 5 * 5, 32 * 5 * 5, 1, 31, 8, 60, 2,
                                       cv::StereoSGBM::MODE_SGBM_3WAY);
        }
        cv::Mat disparity16;
        state.m_lkPerFrameSgbm->compute(state.m_lkPrevLeft, state.m_lkPrevRight, disparity16);
        disparity16.convertTo(disparity, CV_32F, 1.0 / 16.0);
    }
    const auto disparityEnd = std::chrono::steady_clock::now();
    result.disparityMs = std::chrono::duration<double, std::milli>(disparityEnd - disparityStart).count();
    ...
    cv::goodFeaturesToTrack(state.m_lkPrevLeft, rawPreviousPoints, kLkGfttPerFrameMaxCorners,
                            kLkGfttQualityLevel, kLkGfttMinDistancePx, cv::Mat(),
                            kLkGfttBlockSize, false, kLkGfttHarrisK);
    previousPoints =
        SelectGfttPointsGridBalanced(rawPreviousPoints, state.m_lkPrevLeft.size(),
                                     kLkGfttPerFrameMaxCorners, kLkGfttPerFrameMaxCornersPerCell);
    const auto gfttEnd = std::chrono::steady_clock::now();
    result.gfttMs = std::chrono::duration<double, std::milli>(gfttEnd - gfttStart).count();
    ...
    if (result.usedVpiRemap && request.requestVpiLk && !previousPoints.empty()) {
        usedVpiLk = ComputeVpiCudaCurrentPyrLk(state.m_lkPrevLeft, previousPoints, currentPoints,
                                              status, state.m_lkPerFrameVpi);
    }
#endif
    if (!usedVpiLk && !previousPoints.empty()) {
        cv::calcOpticalFlowPyrLK(state.m_lkPrevLeft, result.leftRect, previousPoints, currentPoints,
    ...
    if (useForwardBackwardCheck && !currentPoints.empty()) {
        cv::calcOpticalFlowPyrLK(result.leftRect, state.m_lkPrevLeft, currentPoints, backwardPoints,
                                 backwardStatus, backwardErrors, cv::Size(21, 21), 3);
    }
    const auto flowEnd = std::chrono::steady_clock::now();
    result.flowMs = std::chrono::duration<double, std::milli>(flowEnd - flowStart).count();
    ...
    result.observations = BuildKltPerFramePnpObservations(observationOptions);
    result.currentLeftPoints = std::move(currentPoints);
    const auto candidateEnd = std::chrono::steady_clock::now();
    result.candidateMs = std::chrono::duration<double, std::milli>(candidateEnd - candidateStart).count();
    return result;
}
                                       ...
                                       int inlierCount)
{
    return !frontend.useKeyframeReference ||
           (state.m_lkFrameCount % static_cast<uint32_t>(std::max(1, frontend.keyframeInterval))) == 0 ||
