...
void SetStereoFeatureEnvDefault(const char *name, const char *legacyName,
                                const char *value) {
  if (!EnvVarIsUnsetOrEmpty(name) || !EnvVarIsUnsetOrEmpty(legacyName)) {
    return;
  }
  SetEnvIfUnset(name, value);
}
...
void ConfigureLightGlueFrontendDefaults(
  ...
  if (!EnvVarIsUnsetOrEmpty("SMART_DRONE_SUPERPOINT_TRT_ENGINE") ||
  ...
  if (config.inputMaxWidth > 0 && config.inputMaxHeight > 0) {
    engineNames.push_back(
        "superpoint_dense_" + std::to_string(config.inputMaxWidth) + "x" +
        std::to_string(config.inputMaxHeight) + "_fp16.engine");
    engineNames.push_back(
        "superpoint_dense_" + std::to_string(config.inputMaxWidth) + "x" +
        std::to_string(config.inputMaxHeight) + "_fp32.engine");
  }
  ...
  for (const std::string &engineName : engineNames) {
    ...
    if (std::filesystem::exists(engine, ec)) {
      SetEnvIfUnset("SMART_DRONE_SUPERPOINT_TRT_ENGINE",
                    engine.string().c_str());
      return;
    }
  }
