...
bool LoadStereoCalibrationFromSettings(const std::string &settingsPath, StereoCalibration &calibration)
{
    ...
    if (settingsPath.empty()) {
        std::cerr << "[stereo_calib] settings path empty; stereo calibration unavailable\n";
        return false;
    }
    ...
    calibration.left.K = MakeCameraMatrixLocal(calibration.left.fx, calibration.left.fy,
                                               calibration.left.cx, calibration.left.cy);
    calibration.right.K = MakeCameraMatrixLocal(calibration.right.fx, calibration.right.fy,
                                                calibration.right.cx, calibration.right.cy);
    calibration.left.D = MakeDistCoeffsLocal(static_cast<float>(fs["Camera1.k1"]),
                                             static_cast<float>(fs["Camera1.k2"]),
                                             static_cast<float>(fs["Camera1.p1"]),
                                             static_cast<float>(fs["Camera1.p2"]));
    calibration.right.D = MakeDistCoeffsLocal(static_cast<float>(fs["Camera2.k1"]),
    ...
    if (calibration.T_c1_c2.empty() || calibration.T_c1_c2.rows != 4 ||
        ...
        std::cerr << "[stereo_calib] Stereo.T_c1_c2 missing; falling back to Camera.bf baseline\n";
    }
    ...
    calibration.loaded = ValidIntrinsics(calibration.left) && ValidIntrinsics(calibration.right);
...
bool EnsureStereoRectifier(StereoCalibration &calibration, const cv::Size &inputSize)
{
    ...
    cv::Mat t = (cv::Mat_<double>(3, 1) << -static_cast<double>(calibration.baselineMeters), 0.0, 0.0);
    if (!calibration.T_c1_c2.empty()) {
        cv::Mat T64;
        calibration.T_c1_c2.convertTo(T64, CV_64F);
        cv::Mat Tlr = T64.inv();
    ...
    cv::stereoRectify(calibration.left.K, calibration.left.D, calibration.right.K, calibration.right.D,
                      inputSize, R, t, R1, R2, P1, P2, Q, cv::CALIB_ZERO_DISPARITY, -1.0, inputSize);
    cv::initUndistortRectifyMap(calibration.left.K, calibration.left.D, R1, P1, inputSize,
                                CV_32FC1, calibration.rectification.leftMapX,
                                calibration.rectification.leftMapY);
    cv::initUndistortRectifyMap(calibration.right.K, calibration.right.D, R2, P2, inputSize,
                                CV_32FC1, calibration.rectification.rightMapX,
                                calibration.rectification.rightMapY);
    ...
    calibration.baselineMeters = std::abs(static_cast<float>(P2.at<double>(0, 3) / P2.at<double>(0, 0)));
                              ...
                              const cv::Mat &rightGray, cv::Mat &leftRect, cv::Mat &rightRect)
{
    ...
    if (calibration.rectification.leftMapX.empty() || calibration.rectification.rightMapX.empty()) {
        return false;
    }
