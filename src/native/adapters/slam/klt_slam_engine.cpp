...
KltSlamEngine::KltSlamEngine(std::string settingsPath)
    : m_state(std::make_unique<SlamModeSharedState>()), m_settingsPath(std::move(settingsPath))
{
}
...
void KltSlamEngine::SetExternalFeatureFrontendClient(ExternalFeatureFrontendClient *client)
{
    if (m_state != nullptr) {
        m_state->m_externalFeatureFrontendClient = client;
    }
}

void KltSlamEngine::SetExternalFeatureInputSizeLimit(int maxWidth, int maxHeight)
{
    if (m_state != nullptr) {
        m_state->m_externalFeatureInputMaxWidth = std::max(0, maxWidth);
        m_state->m_externalFeatureInputMaxHeight = std::max(0, maxHeight);
    }
}

void KltSlamEngine::SetStereoVoLoopClosure(bool enabled, float scale, float relaxation)
{
    if (m_state == nullptr) {
        return;
    }
    m_state->m_lkLoopClosureEnabled = enabled;
    m_state->m_lkLoopScale = std::clamp(scale, 0.25f, 4.0f);
    m_state->m_lkLoopRelaxation = std::clamp(relaxation, 0.0f, 4.0f);
}
...
core::ports::SlamOutput KltSlamEngine::Process(const core::ports::SlamInputBatch &input, bool extractFeatures,
...
core::ports::SlamOutput KltSlamEngine::ProcessContinuousKlt(const core::ports::SlamInputBatch &input,
                                                            bool extractFeatures)
{
    ...
    KltContinuousFrontendResult frontend =
        RunKltContinuousFrontend(state, input.stereo.left.gray, input.stereo.right.gray);
    if (!frontend.valid) {
    ...
    if (!frontend.havePreviousFrame) {
        RefreshLkStereoSeedsIfNeeded(state, frontend.leftRect, frontend.rightRect, input.frameId, true);
        state.m_lkTracks = SelectLkTracksGridBalanced(state.m_lkTracks, frontend.leftRect.size());
        ...
        std::vector<cv::Point3f> objectPoints = std::move(frontend.observations.pnp.objectPoints);
        std::vector<cv::Point2f> imagePoints = std::move(frontend.observations.pnp.imagePoints);
        std::vector<LkStereoTrack> trackedTracks = std::move(frontend.observations.trackedTracks);

        const KltPnpCameraIntrinsics camera{state.m_lkFx, state.m_lkFy, state.m_lkCx, state.m_lkCy};
        ...
        std::vector<LkStereoTrack> inlierTracks = KeepLkTracksByIndices(trackedTracks, poseEstimate.inlierIndices);
        ...
        UpdateLkTracksAfterPoseEstimate(state, frontend.leftRect, frontend.rightRect, input.frameId,
    ...
    const Sophus::SE3f outputTwc =
        ApplyKltLoopClosure(state, frontend.leftRect, input.frameId, state.m_lkTwc);
    state.CopyExternalFeatureStatsToOutput(out);
    out.pose = PoseFromTwc(outputTwc);
    return out;
}

core::ports::SlamOutput KltSlamEngine::ProcessPerFrameKlt(const core::ports::SlamInputBatch &input,
                                                          bool extractFeatures)
{
    SlamModeSharedState &state = *m_state;
    core::ports::SlamOutput out = MakeOkSlamOutput(input);
    ...
    KltPerFrameFrontendResult frontend =
        RunKltPerFrameFrontend(state, input.stereo.left.gray, input.stereo.right.gray);
    ...
    if (!state.m_lkHavePrev) {
        ...
        std::vector<cv::Point3f> objectPoints = std::move(frontend.observations.objectPoints);
        std::vector<cv::Point2f> imagePoints = std::move(frontend.observations.imagePoints);
