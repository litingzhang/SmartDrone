...
KltPnpPoseEstimatorOptions MakeKltContinuousPnpPoseEstimatorOptions(
    const KltPnpCameraIntrinsics &camera, bool horizontalLateralFlow)
{
...
KltPnpPoseEstimatorOptions MakeKltPerFramePnpPoseEstimatorOptions(
    const KltPnpCameraIntrinsics &camera, bool preferVpiDefaults)
{
    KltPnpPoseEstimatorOptions options;
    options.camera = camera;
    options.minPoints = kLkMinPnPPoints;
    ...
    const double defaultReprojection =
        preferVpiDefaults ? kLkPerFrameAcceleratedPnPReprojThresholdPx : kLkPerFramePnPReprojThresholdPx;
    const char *reprojectionOverride = std::getenv("SMART_DRONE_LK_PER_FRAME_PNP_REPROJ");
...
KltPnpPoseEstimateResult EstimateKltPnpPoseDelta(
    ...
    const KltPnpPoseEstimatorOptions &options)
{
    ...
    pnpOptions.cameraMatrix =
        MakeCameraMatrix(options.camera.fx, options.camera.fy, options.camera.cx, options.camera.cy);
    ...
    if (!EstimateVisualPnpPoseRansac(objectPoints, imagePoints, pnpOptions, pnpResult)) {
        return result;
    }
    ...
    result.deltaTwc =
        StabilizeLkCameraDelta(pnpResult.T_camera_object.inverse(), options.horizontalLateralFlow);
