...
void PrepareStereoPairForSlam(
    const ports::StereoFrame& stereo,
    double meanL,
    double stdL,
    double meanR,
    double stdR,
...
AutoSlamModeController::SlamOperationMode AutoSlamModeController::Observe(
    bool trackingUsable,
    PoseQuality quality,
    double frameGapMs,
    ...
    std::atomic<bool>& runningFlag)
{
    ...
    auto slamSystem = std::make_unique<ORB_SLAM3::System>(cfg.app.vocab, cfg.app.settings, orbSensor, false);
    ...
    while (runningFlag.load() && !stop.load()) {
        ...
        lastFrameNs = frameNs;
        PrepareStereoPairForSlam(stereoBatch.stereo, meanL, stdL, meanR, stdR, a.slamLowLightEnhance, slamInput.stereo);
        ...
        const uint64_t slamInputTimestampNs =
            static_cast<uint64_t>(std::chrono::duration_cast<std::chrono::nanoseconds>(slamStartTp.time_since_epoch()).count());
        ...
        const uint64_t slamOutputTimestampNs =
            static_cast<uint64_t>(std::chrono::duration_cast<std::chrono::nanoseconds>(slamEndTp.time_since_epoch()).count());
