...
bool SanitizeImuWindow(std::vector<smartdrone::core::ports::ImuReading> &vImu,
  ...
  stats.firstLeadSec =
      static_cast<double>(vImu.front().timestampNs) * 1e-9 - prevFrameTime;
  stats.tailLagSec =
      frameTime - static_cast<double>(vImu.back().timestampNs) * 1e-9;
...
MainRuntimeAliases BuildRuntimeAliases(const AppConfig &c) {
  ...
  a.uvcEyeWidth =
      c.camera.uvcEyeWidth > 0 ? c.camera.uvcEyeWidth : c.camera.width;
  a.uvcEyeHeight =
      c.camera.uvcEyeHeight > 0 ? c.camera.uvcEyeHeight : c.camera.height;
  ...
  a.visualFeatureInputMaxWidth = c.runtime.visualFeatureInputMaxWidth;
  a.visualFeatureInputMaxHeight = c.runtime.visualFeatureInputMaxHeight;
...
void PrintStartupConfig(const AppConfig &app, const MainRuntimeAliases &a,
                        ControllerMode mode) {
  ...
  std::cerr << "cam " << a.width << "x" << a.height << " @" << a.fps
            ...
            << " exp_us=" << a.exposureUs << " gain=" << a.gain
  ...
  std::cerr << "stereo_input eye=" << a.uvcEyeWidth << "x" << a.uvcEyeHeight
            ...
            << " pair_window_ms=" << a.pairMs << " keep_window_ms=" << a.keepMs
            << " frame_queue=" << a.pairQueue << "\n";
  if (packedStereo) {
    std::cerr << "stereo_input_note=single_uvc_frame_split_left_right_no_"
                 "timestamp_pairing\n";
  }
  std::cerr << "slam_input_fps=" << a.slamInputFps << " camera_fps=" << a.fps
            << " frame_drop=" << (a.slamInputFps < a.fps ? "Y" : "N") << "\n";
  std::cerr << "slam_mode="
            << smartdrone::core::domain::ToString(a.slamOperationMode) << "\n";
  ...
  if (a.slamBackend == SlamBackend::DpvoTensorRt) {
    std::cerr << "dpvo_tensorrt repo=" << app.runtime.dpvoRepo
              ...
              << " input=" << app.runtime.dpvoInputWidth << "x"
              << app.runtime.dpvoInputHeight
              << " patches=" << app.runtime.dpvoPatchesPerFrame
              << " opt_window=" << app.runtime.dpvoOptimizationWindow << "\n";
  }
  if (IsVisualFeatureLightGlueFrontend(a.featureFrontend)) {
    std::cerr << "visual_feature_frontend repo="
              << app.runtime.visualFeatureRepo
              << " device=" << app.runtime.visualFeatureDevice
              << " top_k=" << app.runtime.visualFeatureTopK
              << " max_points=" << app.runtime.visualFeatureMaxPoints
              << " input_max=" << app.runtime.visualFeatureInputMaxWidth << "x"
  ...
  std::cerr << "lk seed=gftt"
            ...
            << "\n";
  std::cerr << "debug right_only_features="
            << (a.debugRightOnlyFeatures ? "Y" : "N") << "\n";
