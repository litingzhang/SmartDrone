...
std::vector<domain::ConfigDescriptor> ConfigRegistry::DefaultDescriptors() {
  return {
      Make(kCameraExposureUs, "Camera exposure time in microseconds", false,
           true, true),
      Make(kCameraGain, "Camera analog gain", false, true, true),
      Make(kCameraAutoExposure,
           "Enable camera auto exposure (CM5 ISP/libcamera controls)", false,
           true, true),
      Make(kCameraPairWindowMs,
           "Stereo pairing window in milliseconds for providers that pair "
           "separate left/right streams",
           false, true, true),
      Make(kCameraUvcDeviceIndex,
           "Single UVC device index for packed stereo capture", false, true,
           true),
      Make(kCameraUvcEyeWidth, "Per-eye width for packed UVC stereo frames",
           false, true, true),
      Make(kCameraUvcEyeHeight, "Per-eye height for packed UVC stereo frames",
           false, true, true),
      Make(kCameraUvcPackedStereo,
           "Expect one UVC frame containing left-right packed stereo images",
           false, true, true),
      Make(kSlamInputFps, "Input frame rate delivered to the SLAM engine", true,
           false, false),
      Make(kSlamBackend,
           "SLAM backend selection: klt, dpvo_tensorrt, or orbslam3 when "
           "compiled",
           false, true, false),
      Make(kSlamFeatureFrontend,
           "Feature frontend selection such as lk_gftt_per_frame, lk, orb, "
           "superpoint_lightglue, or xfeat_lightglue",
           false, true, false),
      Make(kSlamPerceptionMode,
           "SLAM perception mode such as stereo or stereo-imu", false, true,
           false),
      Make(kSlamOperationMode,
           "SLAM operating mode such as mapping or localization", true, false,
           false),
      Make(kSlamUseCustomTbc,
           "Override T_b_c1 using runtime-configured translation and "
           "roll/pitch/yaw",
           true, false, false),
      Make(kSlamTbcTx, "Runtime T_b_c1 override translation X (meters)", true,
           false, false),
      Make(kSlamTbcTy, "Runtime T_b_c1 override translation Y (meters)", true,
           false, false),
      Make(kSlamTbcTz, "Runtime T_b_c1 override translation Z (meters)", true,
           false, false),
      Make(kSlamTbcRollDeg,
           "Runtime T_b_c1 override camera roll angle (degrees)", true, false,
           false),
      Make(kSlamTbcPitchDeg,
           "Runtime T_b_c1 override camera pitch angle (degrees)", true, false,
           false),
      Make(kSlamTbcYawDeg, "Runtime T_b_c1 override camera yaw angle (degrees)",
           true, false, false),
      Make(kSlamOrbNFeatures, "ORB extractor max features per frame", false,
           true, false),
      Make(kSlamOrbScaleFactor, "ORB extractor pyramid scale factor", false,
           true, false),
      Make(kSlamOrbNLevels, "ORB extractor pyramid levels", false, true, false),
      Make(kSlamOrbIniThFast, "ORB extractor initial FAST threshold", false,
           true, false),
      Make(kSlamOrbMinThFast, "ORB extractor minimum FAST threshold", false,
           true, false),
      Make(kSlamVisualFeatureTopK,
           "Visual feature detector top-k candidate count before filtering",
           false, true, false),
      Make(kSlamVisualFeatureMaxPoints,
           "Visual feature maximum injected feature count per eye", false, true,
           false),
      Make(kSlamSuperPointTopK,
           "Deprecated alias for slam.visual_feature_top_k", false, true,
           false),
      Make(kSlamSuperPointMaxPoints,
           "Deprecated alias for slam.visual_feature_max_points", false, true,
           false),
      Make(kSlamVisualFeatureInputMaxWidth,
           "Visual feature frontend input maximum width; zero disables "
           "width-based downscaling",
           false, true, false),
      Make(kSlamVisualFeatureInputMaxHeight,
           "Visual feature frontend input maximum height; zero disables "
           "height-based downscaling",
           false, true, false),
      Make(kSlamSuperPointInputMaxWidth,
           "Deprecated alias for slam.visual_feature_input_max_width", false,
           true, false),
      Make(kSlamSuperPointInputMaxHeight,
           "Deprecated alias for slam.visual_feature_input_max_height", false,
           true, false),
      Make(kSlamLkSuperPointSeeding,
           "Deprecated; LK always uses GFTT/Shi-Tomasi seed points", false,
           true, false),
      Make(kSlamLkPerFrameAcceleration,
           "Acceleration backend for KLT tracking mode: cpu or vpi-cuda", false,
           true, false),
      Make(kSlamOrbAcceleration,
           "Acceleration backend for ORB: cpu, cuda, or vpi-remap", false, true,
           false),
      Make(kStreamUdpEnabled, "Enable UDP preview and telemetry streaming",
           false, true, false),
      Make(kStreamUdpIp, "Destination IP for UDP preview streaming", false,
           true, false),
      Make(kStreamSendImage, "Enable image preview streaming", false, true,
           false),
      Make(kStreamSendFeature, "Enable tracked feature streaming", false, true,
           false),
      Make(kStreamSendMap, "Enable pose and map streaming", false, true, false),
  };
}

domain::ConfigDescriptor ConfigRegistry::Make(std::string_view key,
