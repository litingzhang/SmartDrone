...
std::vector<domain::ConfigDescriptor> ConfigRegistry::DefaultDescriptors()
{
    return {
        ...
        Make(kSlamXFeatTopK, "SuperPoint detector top-k candidate count before filtering", false, true, false),
        Make(kSlamXFeatMaxPoints, "SuperPoint maximum injected feature count per eye", false, true, false),
        Make(kSlamXFeatInputMaxWidth, "SuperPoint input maximum width; zero disables width-based downscaling", false,
             true, false),
        Make(kSlamXFeatInputMaxHeight, "SuperPoint input maximum height; zero disables height-based downscaling",
             false, true, false),
        Make(kSlamLkXFeatSeeding, "Deprecated; LK always uses GFTT/Shi-Tomasi seed points", false, true, false),
