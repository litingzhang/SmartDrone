...
std::vector<domain::ConfigDescriptor> ConfigRegistry::DefaultDescriptors()
{
    return {
        ...
        Make(kCameraPairWindowMs, "Stereo pairing window in milliseconds for providers that pair separate left/right streams",
             false, true, true),
        Make(kCameraUvcDeviceIndex, "Single UVC device index for packed stereo capture", false, true, true),
        Make(kCameraUvcEyeWidth, "Per-eye width for packed UVC stereo frames", false, true, true),
        Make(kCameraUvcEyeHeight, "Per-eye height for packed UVC stereo frames", false, true, true),
        ...
        Make(kSlamBackend, "SLAM backend selection: klt, dpvo_tensorrt, or orbslam3 when compiled", false, true, false),
        Make(kSlamFeatureFrontend,
             "Feature frontend selection such as lk_gftt_per_frame, lk, orb, superpoint_lightglue, or xfeat_lightglue",
             false, true, false),
        ...
        Make(kSlamSuperPointTopK, "SuperPoint detector top-k candidate count before filtering", false, true, false),
        Make(kSlamSuperPointMaxPoints, "SuperPoint maximum injected feature count per eye", false, true, false),
        Make(kSlamSuperPointInputMaxWidth, "SuperPoint input maximum width; zero disables width-based downscaling", false,
             true, false),
        Make(kSlamSuperPointInputMaxHeight, "SuperPoint input maximum height; zero disables height-based downscaling",
             false, true, false),
        Make(kSlamLkSuperPointSeeding, "Deprecated; LK always uses GFTT/Shi-Tomasi seed points", false, true, false),
        Make(kSlamLkPerFrameAcceleration, "Acceleration backend for KLT tracking mode: cpu or vpi-cuda", false,
             true, false),
