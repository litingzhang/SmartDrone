...
std::vector<domain::ConfigDescriptor> ConfigRegistry::DefaultDescriptors()
{
    return {
        ...
        Make(kSlamXFeatTopK, "XFeat detector top-k candidate count before filtering", false, true, false),
        Make(kSlamXFeatMaxPoints, "XFeat maximum injected feature count per eye", false, true, false),
        Make(kSlamXFeatInputMaxWidth, "XFeat input maximum width; zero disables width-based downscaling", false, true,
             false),
        Make(kSlamXFeatInputMaxHeight, "XFeat input maximum height; zero disables height-based downscaling", false,
             true, false),
        Make(kSlamLkXFeatSeeding, "Use XFeat stereo matches as LK seed points instead of GFTT/Shi-Tomasi", false,
             true, false),
