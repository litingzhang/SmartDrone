...
RouteResult HandleRuntimeConfigFrame(const TlvFrame &frame, const UdpPeer &peer,
  ...
  if (frame.len != RUNTIME_CONFIG_PAYLOAD_LEN_V14 &&
      ...
      frame.len != RUNTIME_CONFIG_PAYLOAD_LEN_V10 &&
      frame.len != RUNTIME_CONFIG_PAYLOAD_LEN_V9 &&
  ...
  r.visualFeatureTopK = currentCfg.app.runtime.visualFeatureTopK;
  r.visualFeatureMaxPoints = currentCfg.app.runtime.visualFeatureMaxPoints;
  r.visualFeatureInputMaxWidth =
      currentCfg.app.runtime.visualFeatureInputMaxWidth;
  r.visualFeatureInputMaxHeight =
  ...
  if (frame.len >= RUNTIME_CONFIG_PAYLOAD_LEN_V2) {
    const int pairMs =
        static_cast<int>(ReadU16Le(&p[RUNTIME_CONFIG_PAIR_MS_OFFSET]));
    if (pairMs > 0)
      r.pairMs = pairMs;
    r.slamInputFps =
        static_cast<int>(ReadU16Le(&p[RUNTIME_CONFIG_SLAM_FPS_OFFSET]));
  ...
  if (frame.len >= RUNTIME_CONFIG_PAYLOAD_LEN_V3) {
    r.slamOperationMode =
        ParseRuntimeSlamMode(p[RUNTIME_CONFIG_SLAM_MODE_OFFSET]);
  }
  ...
  if (frame.len >= RUNTIME_CONFIG_PAYLOAD_LEN_V7) {
    r.orbNFeatures = static_cast<int>(
        std::lround(ReadF32Le(&p[RUNTIME_CONFIG_ORB_NFEATURES_OFFSET])));
    r.orbScaleFactor = ReadF32Le(&p[RUNTIME_CONFIG_ORB_SCALE_FACTOR_OFFSET]);
    r.orbNLevels = static_cast<int>(
        std::lround(ReadF32Le(&p[RUNTIME_CONFIG_ORB_NLEVELS_OFFSET])));
    r.orbIniThFAST = static_cast<int>(
        std::lround(ReadF32Le(&p[RUNTIME_CONFIG_ORB_INI_TH_FAST_OFFSET])));
    r.orbMinThFAST = static_cast<int>(
        std::lround(ReadF32Le(&p[RUNTIME_CONFIG_ORB_MIN_TH_FAST_OFFSET])));
  }
  if (frame.len >= RUNTIME_CONFIG_PAYLOAD_LEN_V8) {
    r.featureFrontend =
        ParseRuntimeFeatureFrontend(p[RUNTIME_CONFIG_FEATURE_FRONTEND_OFFSET]);
  }
  if (frame.len >= RUNTIME_CONFIG_PAYLOAD_LEN_V9) {
    r.visualFeatureTopK = static_cast<int>(
        std::lround(ReadF32Le(&p[RUNTIME_CONFIG_SUPERPOINT_TOP_K_OFFSET])));
    r.visualFeatureMaxPoints = static_cast<int>(std::lround(
        ReadF32Le(&p[RUNTIME_CONFIG_SUPERPOINT_MAX_POINTS_OFFSET])));
  }
  if (frame.len >= RUNTIME_CONFIG_PAYLOAD_LEN_V10) {
    r.visualFeatureInputMaxWidth = static_cast<int>(std::lround(
        ReadF32Le(&p[RUNTIME_CONFIG_SUPERPOINT_INPUT_MAX_WIDTH_OFFSET])));
    r.visualFeatureInputMaxHeight = static_cast<int>(std::lround(
        ReadF32Le(&p[RUNTIME_CONFIG_SUPERPOINT_INPUT_MAX_HEIGHT_OFFSET])));
  }
  // Offset 106 is retained for wire compatibility; LK seeding is always GFTT.
  if (frame.len >= RUNTIME_CONFIG_PAYLOAD_LEN_V12) {
    r.lkPerFrameAcceleration = ParseRuntimeLkPerFrameAcceleration(
        p[RUNTIME_CONFIG_LK_PER_FRAME_ACCEL_OFFSET]);
  }
  if (frame.len >= RUNTIME_CONFIG_PAYLOAD_LEN_V13) {
    r.orbAcceleration =
        ParseRuntimeOrbAcceleration(p[RUNTIME_CONFIG_ORB_ACCEL_OFFSET]);
  }
  if (frame.len >= RUNTIME_CONFIG_PAYLOAD_LEN_V14) {
    r.slamBackend =
        ParseRuntimeSlamBackend(p[RUNTIME_CONFIG_SLAM_BACKEND_OFFSET]);
  }
  ...
  update.values[std::string(ConfigRegistry::kCameraExposureUs)] =
      static_cast<int64_t>(r.exposureUs);
  ...
  return {
      ACK_OK,
      result.message + " udp=" + r.udpIp + " settings=" +
          std::string(DefaultSettingsForSensorMode(r.sensorMode)) +
          ...
          " ae=" + (r.autoExposureEnabled ? "on" : "off") +
          " tbc_override=" + (r.useCustomTbc ? "on" : "off") + " lk_seed=gftt" +
...
HandleGetConfigFrame(const TlvFrame &frame,
  ...
  out.responsePayload =
      buildConfigPayload ? buildConfigPayload(controller.CurrentConfig(),
                                              controller.CurrentDesiredMode())
                         : std::vector<uint8_t>{};
  return out;
}
...
std::thread StartUdpCommandThread(
    int port, Px4UdpHooks &hooks, UnifiedRuntimeController &controller,
  ...
  return smartdrone::common::StartThread(
      smartdrone::common::MakeThreadLaunchInfo(
      ...
      [port, &hooks, &controller, &livePose, &runningFlag,
       ...
       peerToIpString = std::move(peerToIpString)]() {
        ...
        const int statePeriodMs = EnvIntValueClamped(
            "SMART_DRONE_UDP_STATE_PERIOD_MS", 100, 20, 1000);
        ...
        while (runningFlag.load()) {
          ...
          if (n <= 0) {
            ...
            if (!peerGate.Accept(peer, now)) {
              if (now - lastRejectedPeerLog >= std::chrono::seconds(1)) {
                ...
                std::cerr
                    << "[udp_cmd] rejected packet from non-active peer ip="
                    << (peerToIpString ? peerToIpString(peer) : std::string{})
            ...
            while (auto frame = parser.TryPop()) {
              ...
              std::vector<uint8_t> ack = MakeAckFrame(
                  frame->seq, frame->tMs, frame->cmd, frame->seq, rr.status);
              server.SendTo(peer, ack.data(), ack.size());
              if (rr.status == ACK_OK && rr.responseCmd != 0) {
                const uint8_t *payload = rr.responsePayload.empty()
                                             ? nullptr
                                             : rr.responsePayload.data();
                std::vector<uint8_t> responseFrame = MakeFrame(
                    TLV_VER, rr.responseCmd, 0, frame->seq, MonoTimeMs32(),
          ...
          const auto now = std::chrono::steady_clock::now();
          if (activePeer.valid &&
              (lastHeartbeatTx.time_since_epoch().count() == 0 ||
               (now - lastHeartbeatTx) >= kHeartbeatPeriod)) {
            lastHeartbeatTx = now;
            std::vector<uint8_t> heartbeatFrame = MakeFrame(
                TLV_VER, kCmdHeartbeat, 0, 0, MonoTimeMs32(), nullptr, 0);
            server.SendTo(activePeer, heartbeatFrame.data(),
                          heartbeatFrame.size());
          }
          LivePoseState::Snapshot heartbeatSnapshot{};
          const bool vehicleArmed = livePose.ReadSnapshot(heartbeatSnapshot) &&
                                    heartbeatSnapshot.armed;
          ...
          if (now - lastStateTx >= std::chrono::milliseconds(statePeriodMs)) {
            ...
            if (livePose.ReadSnapshot(snap) && snap.hasPeer) {
              ...
              std::vector<uint8_t> stateFrame = MakeFrame(
                  TLV_VER, CMD_STATE, 0, snap.seq, MonoTimeMs32(),
              ...
              if (currentCfg.app.udp.sendMap) {
                ...
                const size_t pointCount =
                    pointCloudXyz ? (pointCloudXyz->size() / 3) : 0;
                if (pointCount > 0 &&
                    snap.pointCloudSeq != lastSentPointCloudSeq) {
                  ...
                  cloudPayload.reserve(kPointCloudHeaderLen +
                                       cappedPointCount *
                                           kPointCloudPointStrideBytes);
                  WriteU16Le(cloudPayload,
                             static_cast<uint16_t>(cappedPointCount));
                  WriteU16Le(cloudPayload, static_cast<uint16_t>(
                                               snap.pointCloudSeq & 0xFFFFu));
                  for (size_t i = 0; i < cappedPointCount * 3; ++i) {
                    WriteF32Le(cloudPayload, (*pointCloudXyz)[i]);
                  }
                  std::vector<uint8_t> cloudFrame =
                      MakeFrame(TLV_VER, kCmdPointCloud, 0, snap.seq,
                                MonoTimeMs32(), cloudPayload.data(),
                                static_cast<uint16_t>(cloudPayload.size()));
                  server.SendTo(snap.peer, cloudFrame.data(),
                                cloudFrame.size());
                  lastSentPointCloudSeq = snap.pointCloudSeq;
                  if (cappedPointCount < pointCount) {
                    std::cerr << "[udp_cmd] point cloud truncated points="
                              << pointCount << " sent=" << cappedPointCount
                              << "\n";
                  }
                }
              }
