...
BuildConfigPayload(const UnifiedConfig &cfg,
  ...
  oss << "camera.auto_exposure="
      << (cfg.app.camera.aeDisable ? "false" : "true") << "\n";
  oss << "camera.auto_exposure_note="
      << (CompiledCameraProviderUsesPackedStereo()
  ...
  oss << "camera.uvc_packed_stereo="
      << (cfg.app.camera.uvcPackedStereo ? "true" : "false") << "\n";
  ...
  oss << "slam.tbc_override_enabled="
      << (cfg.app.runtime.useCustomTbc ? "true" : "false") << "\n";
