...
bool RuntimeConfigService::UpdateRemoteConfig(RemoteRuntimeConfig remote,
  ...
  if (remote.visualFeatureTopK < 1 || remote.visualFeatureTopK > 4096 ||
    ...
    if (err) {
      *err = "visual feature config out of range";
    }
    return false;
  }
  if (remote.visualFeatureInputMaxWidth < 0 ||
    ...
    if (err) {
      *err = "visual feature input size config out of range";
    }
    return false;
  }
  ...
  float effectiveTbcYawDeg = remote.tbcYawDeg;
  {
    ...
    const bool orbParamsAffectPipeline =
        m_config.app.runtime.slamBackend == SlamBackend::OrbSlam3 ||
        remote.slamBackend == SlamBackend::OrbSlam3;
    const bool orbChanged =
        orbParamsAffectPipeline &&
        (m_config.app.runtime.orbNFeatures != remote.orbNFeatures ||
         std::abs(m_config.app.runtime.orbScaleFactor - remote.orbScaleFactor) >
             1e-6f ||
    ...
    const bool visualFeatureConfigChanged =
        m_config.app.runtime.visualFeatureTopK != remote.visualFeatureTopK ||
        m_config.app.runtime.visualFeatureMaxPoints !=
            remote.visualFeatureMaxPoints ||
        m_config.app.runtime.visualFeatureInputMaxWidth !=
            remote.visualFeatureInputMaxWidth ||
        m_config.app.runtime.visualFeatureInputMaxHeight !=
            remote.visualFeatureInputMaxHeight;
    const bool lkPerFrameAccelChanged =
        m_config.app.runtime.lkPerFrameAcceleration !=
    ...
    m_config.app.runtime.orbMinThFAST = remote.orbMinThFAST;
    m_config.app.runtime.visualFeatureTopK = remote.visualFeatureTopK;
    m_config.app.runtime.visualFeatureMaxPoints = remote.visualFeatureMaxPoints;
    m_config.app.runtime.visualFeatureInputMaxWidth =
        remote.visualFeatureInputMaxWidth;
    m_config.app.runtime.visualFeatureInputMaxHeight =
        remote.visualFeatureInputMaxHeight;
    ...
    m_config.app.settings =
        ResolveSettingsForSensorMode(remote.sensorMode, m_config.app.settings);
    ...
    restartNeeded = sensorModeChanged || backendChanged || frontendChanged ||
                    aeModeChanged || uvcConfigChanged || orbChanged ||
                    visualFeatureConfigChanged || lkPerFrameAccelChanged ||
                    orbAccelChanged;
  }
  ...
  m_tuning.slamOperationMode.store(
      static_cast<uint8_t>(remote.slamOperationMode),
      std::memory_order_relaxed);
  m_tuning.featureFrontend.store(static_cast<uint8_t>(remote.featureFrontend),
                                 std::memory_order_relaxed);
  ...
  for (const auto &[key, value] : update.values) {
    if (key == ConfigRegistry::kCameraExposureUs) {
      ...
      if (const auto *v = std::get_if<std::string>(&value)) {
        remote.slamBackend =
            NormalizeSlamBackendForBuild(ParseSlamBackendText(*v));
      } else {
    ...
    } else if (key == ConfigRegistry::kSlamVisualFeatureTopK ||
               key == ConfigRegistry::kSlamSuperPointTopK) {
      if (const auto *v = std::get_if<int64_t>(&value)) {
        remote.visualFeatureTopK = static_cast<int>(*v);
      } else if (const auto *vFloat = std::get_if<double>(&value)) {
        remote.visualFeatureTopK = static_cast<int>(*vFloat);
      } else {
        return {false, "slam.visual_feature_top_k type mismatch"};
      }
    } else if (key == ConfigRegistry::kSlamVisualFeatureMaxPoints ||
               key == ConfigRegistry::kSlamSuperPointMaxPoints) {
      if (const auto *v = std::get_if<int64_t>(&value)) {
        remote.visualFeatureMaxPoints = static_cast<int>(*v);
      } else if (const auto *vFloat = std::get_if<double>(&value)) {
        remote.visualFeatureMaxPoints = static_cast<int>(*vFloat);
      } else {
        return {false, "slam.visual_feature_max_points type mismatch"};
      }
    } else if (key == ConfigRegistry::kSlamVisualFeatureInputMaxWidth ||
               key == ConfigRegistry::kSlamSuperPointInputMaxWidth) {
      if (const auto *v = std::get_if<int64_t>(&value)) {
        remote.visualFeatureInputMaxWidth = static_cast<int>(*v);
      } else if (const auto *vFloat = std::get_if<double>(&value)) {
        remote.visualFeatureInputMaxWidth = static_cast<int>(*vFloat);
      } else {
        return {false, "slam.visual_feature_input_max_width type mismatch"};
      }
    } else if (key == ConfigRegistry::kSlamVisualFeatureInputMaxHeight ||
               key == ConfigRegistry::kSlamSuperPointInputMaxHeight) {
      if (const auto *v = std::get_if<int64_t>(&value)) {
        remote.visualFeatureInputMaxHeight = static_cast<int>(*v);
      } else if (const auto *vFloat = std::get_if<double>(&value)) {
        remote.visualFeatureInputMaxHeight = static_cast<int>(*vFloat);
      } else {
        return {false, "slam.visual_feature_input_max_height type mismatch"};
      }
