...
std::thread StartUdpDiscoveryBeaconThread(int discoveryPort, int cmdPort, int videoPort, std::atomic<bool> &runningFlag)
{
    return smartdrone::common::StartThread(
        smartdrone::common::MakeThreadLaunchInfo(smartdrone::common::ThreadRole::DiscoveryBeacon, "UdpDiscoveryBeacon"),
        [discoveryPort, cmdPort, videoPort, &runningFlag]() {
            ...
            const std::string payload = std::string(kDiscoveryMagic) + ";device=cm5;cmd=" + std::to_string(cmdPort) +
            ...
            while (runningFlag.load()) {
                ...
                if (sent < 0) {
                    ...
                    firstLog = false;
                    std::cerr << "[discovery] broadcasting on udp/" << discoveryPort << " cmd=" << cmdPort
                              << " video=" << videoPort << "\n";
                }
