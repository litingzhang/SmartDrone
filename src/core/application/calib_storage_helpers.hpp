...
inline std::string TsToName(int64_t tNs)
{
    return std::to_string(tNs) + ".png";
}

inline void SetupFileBuffer(FILE* f, size_t bytes)
{
    if (f) setvbuf(f, nullptr, _IOFBF, bytes);
}

inline bool TryParseCalibIndex(const std::string& name, int& indexOut)
{
...
inline std::string MakeCalibSessionDir(const std::string& root)
{
...
inline int CleanupCalibDataDirs(const std::string& root)
{
    int removed = 0;
    std::error_code ec;
