...
inline int ClampSlamInputFps(int requestedFps, int cameraFps)
{
    if (cameraFps <= 0) {
...
inline bool IsFiniteImuPoint(const ORB_SLAM3::IMU::Point& p)
{
...
inline bool SanitizeImuWindow(std::vector<ORB_SLAM3::IMU::Point>& vImu,
...
inline std::optional<Sophus::SE3f> ReadSe3Node(const cv::FileNode& node)
{
...
inline StereoBodyExtrinsics LoadStereoBodyExtrinsics(const std::string& settingsPath)
{
...
inline MainRuntimeAliases BuildRuntimeAliases(const AppConfig& c)
{
